export function parseRoute(hash = location.hash || "#home") {
  ...
  if (path === 'ops') return { name: 'ops', params: { module: rest[0] || 'sales-manager' } };
  if (path === 'sales-manager') return { name: 'ops', params: { module: 'sales-manager' } };
  if (path === 'admin') return { name: 'admin', params: { module: rest[0] || 'products' } };
  if (path === 'rep') return { name: 'rep', params: { module: rest[0] || 'dashboard' } };
  return { name: 'home' };
}
...
export function toHash(routeName, params = {}) {
  ...
  if (routeName === 'ops') return `#ops/${encodeURIComponent(params.module || 'sales-manager')}`;
  if (routeName === 'sales-manager') return '#ops/sales-manager';
  if (routeName === 'admin') return `#admin/${encodeURIComponent(params.module || 'products')}`;
  if (routeName === 'rep') {
    const module = params?.module || 'dashboard';
    return module === 'dashboard' ? '#rep' : `#rep/${encodeURIComponent(module)}`;
  }
  return `#${routeName}`;
}
