...
import { computeTeamPerformance } from './performanceService.js';
...
function getOrders(state) {
  ...
  return dedupeById([...priorityOrders, ...teamOrders]);
}
...
function getRepName(order, teamReps) {
  const repId = order?.owner_id || order?.rep_id || order?.sales_rep_id || order?.created_by_rep_id || null;
...
export function buildVisitCoverage(teamReps, teamVisits, teamOrders) {
  var repVisits = {};
  for (var i = 0; i < (Array.isArray(teamVisits) ? teamVisits : []).length; i++) {
    var v = teamVisits[i];
    var rid = String(v.rep_id || '');
    if (!repVisits[rid]) repVisits[rid] = { total: 0, completed: 0, ordered: 0, closed: 0, followUp: 0 };
    repVisits[rid].total++;
    var s = (v.visit_status || '').toLowerCase();
    if (s === 'completed' || s === 'ordered') repVisits[rid].completed++;
    if (s === 'ordered') repVisits[rid].ordered++;
    if (s === 'customer_closed') repVisits[rid].closed++;
    if (s === 'follow_up_required') repVisits[rid].followUp++;
  }

  var repOrderCounts = {};
  for (var j = 0; j < (Array.isArray(teamOrders) ? teamOrders : []).length; j++) {
    var o = teamOrders[j];
    var oid = String(o.owner_id || o.rep_id || o.sales_rep_id || '');
    if (!repOrderCounts[oid]) repOrderCounts[oid] = 0;
    repOrderCounts[oid]++;
  }

  return (Array.isArray(teamReps) ? teamReps : []).map(function(rep) {
    var rid = String(rep.id || '');
    var rv = repVisits[rid] || { total: 0, completed: 0, ordered: 0, closed: 0, followUp: 0 };
    return {
      repId: rid,
      repName: rep.name || rep.username || 'مندوب',
      visits: rv.total,
      completed: rv.completed,
      ordered: rv.ordered,
      closed: rv.closed,
      followUp: rv.followUp,
      orders: repOrderCounts[rid] || 0,
      coverage: rv.total > 0 ? Math.round((rv.completed / rv.total) * 100) : 0,
    };
...
export function createOpsDashboardModel(state) {
  ...
  return {
    ...
    teamVisits: Array.isArray(state?.runtime?.manager?.teamVisits) ? state.runtime.manager.teamVisits : [],
    suspiciousVisits: (Array.isArray(state?.runtime?.manager?.teamVisits) ? state.runtime.manager.teamVisits : []).filter(function(v) {
      return (v.visit_confidence_score != null && v.visit_confidence_score < 50) || v.visit_flags;
    }).map(function(v) {
      var repMap = {};
      var teamReps = state?.runtime?.manager?.teamReps || [];
      teamReps.forEach(function(r) { repMap[String(r.id)] = r.name || r.username || 'مندوب'; });
      var customerMap = {};
      var teamCusts = state?.runtime?.manager?.teamCustomers || [];
      teamCusts.forEach(function(c) { customerMap[String(c.id)] = c.name || 'عميل'; });
      v.rep_name = repMap[String(v.rep_id)] || 'مندوب';
      v.customer_name = customerMap[String(v.customer_id)] || 'عميل';
      return v;
    }),
    teamPerformance: computeTeamPerformance(
      state?.runtime?.manager?.teamReps || [],
      state?.runtime?.manager?.teamVisits || [],
      getOrders(state),
      getCustomers(state),
    ).rankings,
    visitCoverage: buildVisitCoverage(
      state?.runtime?.manager?.teamReps || [],
      state?.runtime?.manager?.teamVisits || [],
      getOrders(state),
    ),
