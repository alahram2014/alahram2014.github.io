...
import { canViewCustomer, canViewInvoice } from './ownershipService.js';
...
function collectLocalCandidates(query, session, state) {
  ...
  for (const p of allProducts) {
    if (fuzzyMatch(p.product_name, q) || fuzzyMatch(p.company_name, q) || fuzzyMatch(p.product_id, q)) {
      filtered.products.push({ id: p.product_id, product_name: p.product_name, company_name: p.company_name, product_image: p.product_image, _type: 'product' });
    }
  }
...
export async function performAiSearch(query, session, state) {
  const q = String(query || '').trim();
  ...
  const local = collectLocalCandidates(q, session, state);
  const allCandidates = [
