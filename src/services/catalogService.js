...
async function loadRowsWithFallback(api, candidates, params = {}, pageSize = 160) {
  const [path] = Array.isArray(candidates) ? candidates : [candidates];
  if (!path) return [];
  return loadPagedRows(api, path, params, pageSize).catch(() => []);
}
...
export async function loadHomeCatalog(api, selectedTierName = null) {
  const requests = await Promise.allSettled([
    api.get('companies', { select: 'company_id,company_name,company_logo,visible,allow_discount', order: 'company_id.asc' }),
    ...
    api.get('v_top_products', { select: 'product_id,product_name,total_qty,total_sales', order: 'total_qty.desc', limit: '12' }),
    api.get('v_top_companies', { select: 'company_id,company_name,total_sales,total_items', order: 'total_sales.desc', limit: '12' }),
  ]);
  ...
  const topProducts = requests[5].status === 'fulfilled' && Array.isArray(requests[5].value) && requests[5].value.length ? requests[5].value : [];
  const topCompanies = requests[6].status === 'fulfilled' && Array.isArray(requests[6].value) && requests[6].value.length ? requests[6].value : [];
...
export async function loadProductsByIds(api, productIds, selectedTierName = null) {
  ...
  if (!ids.length) {
    return {
      ...
      productIndex: {},
      products: [],
      priceBook: buildPriceBook([], [], selectedTierName),
    };
  }

  const rows = await loadRowsWithFallback(api, ['v_runtime_products_mobile'], {
    select: 'product_id,product_name,company_id,company_name,company_logo,category,product_image,status,visible,unit_code,tier_name,final_price,available_qty,reserved_qty,allow_backorder,runtime_healthy,is_sellable,unit_active,min_qty,display_order',
...
export async function updateProduct(api, productId, payload = {}) {
  ...
  const rows = await api.patch('products', cleanPatchPayload(payload), { product_id: `eq.${id}` });
  return Array.isArray(rows) ? rows[0] : rows;
}

export async function hideProduct(api, productId, hidden = true) {
  return updateProduct(api, productId, {
    visible: !hidden,
    status: hidden ? 'inactive' : 'active',
  });
}

export async function updateTierPricing(api, tierName, productId, unitCode, payload = {}) {
  const product = String(productId || '').trim();
  const tier = String(tierName || '').trim();
  const unit = String(unitCode || '').trim();
  if (!product || !tier || !unit) throw new Error('INVALID_PRICING_KEY');
  const rows = await api.patch('pricing_product_prices', cleanPatchPayload(payload), {
