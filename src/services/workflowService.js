...
export function normalizeWorkflowStateKey(value) {
  ...
  if (Object.prototype.hasOwnProperty.call(LEGACY_STATUS_TO_STATE_KEY, lower)) return LEGACY_STATUS_TO_STATE_KEY[lower];
...
export function canUserExecuteTransition() {
  return true;
}
...
export function resolveWorkflowActions(order, session = {}) {
  ...
  return {
    ...
    executableTransitions: allowedTransitions,
    executableTransitionKeys: allowedTransitions.map((transition) => transition.to_state_key),
    capabilities: [],
  };
}
