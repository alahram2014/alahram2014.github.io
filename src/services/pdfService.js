export function generateInvoicePDF(invoice, groupedItems, companies) {
  ...
  const locationHtml = invoice.location_lat && invoice.location_lng && invoice.location_maps_url
    ? `<div class="invoice-location"><span class="location-icon">📍</span><strong>موقع التسليم</strong><a href="${escapeHtml(invoice.location_maps_url)}" target="_blank" rel="noopener" style="color:#0052cc;text-decoration:none;">${escapeHtml(invoice.location_lat)}, ${escapeHtml(invoice.location_lng)}</a><span style="font-size:9pt;color:#888;display:block;margin-top:2px;">${escapeHtml(invoice.location_maps_url)}</span></div>`
    : '';
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .invoice-location { margin-bottom: 20px; padding: 10px 14px; background: #fff8e6; border-radius: 6px; border: 1px solid #f0d78a; font-size: 10pt; line-height: 1.8; text-align: center; page-break-inside: avoid; }
  .invoice-location .location-icon { font-size: 14pt; margin-left: 4px; }
  .invoice-location strong { display: block; margin-bottom: 4px; }
