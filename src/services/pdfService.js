  ...
  for (const group of groupedItems) {
    ...
    for (const item of group.items) {
      ...
      tableRows += `<tr>
        ...
        <td>${escapeHtml(item.unitLabel || item.unit || 'قطعة')}</td>
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .invoice-info { display: flex; justify-content: space-between; margin-bottom: 20px; padding: 12px; background: #f8f9fa; border-radius: 6px; }
...
<div class="invoice-info">
  <div><span>اسم العميل</span><strong>${escapeHtml(String(invoice.customer_name || invoice.name || ''))}</strong></div>
