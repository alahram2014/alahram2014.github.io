...
export function addProductToCart(cart, product, tier, unitPreference, quantity = 1) {
  ...
  if (!canAddProductToCart(product, unit)) {
    return { cart: Array.isArray(cart) ? cart : [], added: false, key: null, reason: 'PRODUCT_UNAVAILABLE' };
  }
...
export function toggleOfferInCart(cart, offer, kind) {
  ...
  if (kind === 'flash' && runtimeStatus !== 'active') {
  ...
  const productId = normalizeOfferProductId(offer);
  const unit = normalizeOfferUnit(offer);
  return {
    cart: [...safeCart, {
      key,
      type: kind,
      id: offer.id,
      offer_id: offer.id,
      product_id: productId || String(offer.id || ''),
      title: offer.title,
      image: offer.image || '',
