import { canAccessCustomerManagement, canAccessOperationalDashboard, hasCapability, hasOperationalAccess as hasCapabilityOperationalAccess, normalizeCapabilityList } from './authService.js';
...
import { loadAuthorizedManagerCustomers, loadAuthorizedManagerOrders } from './ownershipService.js';
import { loadTeamVisits } from './visitService.js';
...
const OPERATIONAL_MODULES = [
  ...
  },
  {
    ...
    description: 'عرض المنتجات في الكتالوج',
    runtimeReady: true,
  },
  {
    key: 'products',
    label: 'المنتجات',
    route: 'ops/products',
    requiredCapabilities: ['products.manage', 'catalog.manage'],
    description: 'إدارة المنتجات والوحدات والتسعير والمخزون',
    runtimeReady: true,
  },
  {
    key: 'companies',
...
function isGlobalOperationalScope(session = {}) {
  return hasCapability(session, ['dashboard.admin']) || hasCapabilityOperationalAccess(session);
}
...
export function hasOperationalAccess(session = {}) {
  return hasCapabilityOperationalAccess(session) || getOperationalModules(session).length > 0;
}
...
export function getOperationalQuickActions(session = {}) {
  const actions = [
    ...
    },
    {
      ...
      visible: hasOperationalAccess(session) || hasCapability(session, ['orders.create']),
    },
    {
      key: 'admin-users',
    ...
    },
    {
      ...
      action: 'go-ops-module',
      module: 'products',
      description: 'إدارة كاملة للمنتجات والتسعير والمخزون',
      visible: hasCapability(session, ['products.manage', 'catalog.manage']),
      enabled: isOperationalModuleReady('products'),
    },
    {
      key: 'company-ops',
      label: 'إدارة الشركات',
...
async function loadManagerCustomers(api, ownerId, session = {}) {
  const result = await loadAuthorizedManagerCustomers(api, session);
...
async function loadManagerReps(api, ownerId, session = {}) {
  ...
  const rows = await api.get('sales_reps', isGlobalOperationalScope(session) ? {
    select: 'id,name,phone,username,region,default_tier_name,is_active,is_blocked,blocked_reason,rep_code,user_type,created_at',
...
async function loadManagerOrders(api, ownerId, customerIds = [], session = {}) {
  return loadAuthorizedManagerOrders(api, session, customerIds);
}
...
export async function loadManagerScope(api, session = {}, { force = false } = {}) {
  ...
  if (cachedScope.loaded && cachedScopeKey === scopeKey && !force) {
    return {
      ...
      teamVisits: Array.isArray(cachedScope.teamVisits) ? cachedScope.teamVisits.map(function(v) { return { ...v }; }) : [],
  ...
  try {
    ...
    var repIds = teamReps.map(function(r) { return r.id; });
    var teamVisits = (repIds.length ? await loadTeamVisits(api, repIds).catch(function() { return []; }) : []);
    ...
    nextScope.teamVisits = Array.isArray(teamVisits) ? teamVisits : [];
    ...
    return {
      ...
      teamVisits: Array.isArray(nextScope.teamVisits) ? nextScope.teamVisits.map(function(v) { return { ...v }; }) : [],
...
export async function loadManagerScopeIntoState(store, api, session = null, { force = false } = {}) {
  ...
  store.update((draft) => {
    draft.runtime.manager = {
      ...
      teamVisits: Array.isArray(scope.teamVisits) ? scope.teamVisits.map(function(v) { return { ...v }; }) : [],
