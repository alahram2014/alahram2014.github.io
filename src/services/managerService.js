import { canAccessCustomerManagement, canAccessOperationalDashboard, hasCapability, hasOperationalAccess as hasCapabilityOperationalAccess, normalizeCapabilityList, normalizeUserType } from './authService.js';
import { publishDomainEvent } from './domainEventService.js';
import { resolveWorkflowActions, getWorkflowStateLabel, normalizeWorkflowStateKey } from './workflowService.js';

const OPERATIONAL_MODULES = [
  {
    key: 'sales-manager',
    label: 'مركز التشغيل',
    route: 'ops/sales-manager',
    requiredCapabilities: ['dashboard.sales_manager', 'dashboard.admin', 'sales_manager.access'],
    description: 'لوحة تشغيلية مختصرة لمتابعة الطلبات والعملاء',
    runtimeReady: true,
  },
  {
    key: 'orders',
    label: 'الطلبات',
    route: 'ops/orders',
    requiredCapabilities: ['orders.view', 'orders.create', 'orders.manage', 'orders.review'],
    description: 'إدارة دورة الطلبات والتنفيذ والتسليم',
    runtimeReady: true,
  },
  {
    key: 'customers',
    label: 'العملاء',
    route: 'ops/customers',
    requiredCapabilities: ['customers.view', 'customers.manage', 'customers.create', 'sales_manager.assign_customers'],
    description: 'إدارة العملاء والملكية والمتابعة',
    runtimeReady: true,
  },
  {
    key: 'reps',
    label: 'المناديب',
    route: 'ops/reps',
    requiredCapabilities: ['sales_manager.manage_reps', 'sales_manager.access', 'dashboard.sales_manager'],
    description: 'متابعة المناديب والعمل التشغيلي للفريق',
    runtimeReady: true,
  },
  {
    key: 'catalog',
    label: 'الكتالوج',
    route: 'ops/catalog',
    requiredCapabilities: ['products.manage', 'catalog.manage', 'pricing.tiers.manage', 'stock.manage'],
    description: 'المنتجات والأسعار والظهور',
    runtimeReady: true,
  },
  {
    key: 'companies',
    label: 'الشركات',
    route: 'ops/companies',
    requiredCapabilities: ['companies.manage', 'company.manage', 'dashboard.admin'],
    description: 'إدارة الشركات والتحكم في الظهور',
    runtimeReady: true,
  },
  {
    key: 'workflow',
    label: 'سير العمل',
    route: 'ops/workflow',
    requiredCapabilities: ['workflow.view', 'workflow.transition', 'workflow.manage', 'orders.review'],
    description: 'مراقبة الحالات والتنقل بين خطوات التنفيذ',
    runtimeReady: true,
  },
  {
    key: 'reports',
    label: 'التقارير',
    route: 'ops/reports',
    requiredCapabilities: ['reports.view', 'dashboard.admin'],
    description: 'تقارير تشغيلية سريعة',
    runtimeReady: true,
  },
  {
    key: 'sales',
    label: 'المبيعات',
    route: 'ops/sales',
    requiredCapabilities: ['orders.view', 'orders.create', 'customers.create', 'customers.manage'],
    description: 'إدارة الطلبات والعملاء المرتبطين بالحساب',
    runtimeReady: true,
  },
];

const WORKFLOW_PRIORITY_RANK = {
  pending: 1,
  reviewing: 2,
  assigned: 2,
  confirmed: 2,
  preparing: 3,
  dispatched: 4,
  shipped: 4,
  delivered: 5,
  collected: 6,
  returned: 7,
  cancelled: 8,
};

function normalizeId(value) {
  return String(value || '').trim();
}

function matchesAnyOwner(row, ownerId, fields = []) {
  const target = normalizeId(ownerId);
  if (!target) return true;
  return fields.some((field) => {
    const value = normalizeId(row?.[field]);
    return Boolean(value) && value === target;
  });
}

function isOperationalRole(session = {}) {
  const type = normalizeUserType(session?.userType || session?.user_type || session?.role || null, null);
  return ['admin', 'sales_manager', 'sales_operator', 'hybrid_operator', 'warehouse_operator', 'operations_manager'].includes(type);
}

function isGlobalOperationalScope(session = {}) {
  return isOperationalRole(session) || hasCapability(session, ['dashboard.admin', 'dashboard.sales_manager', 'sales_manager.access']);
}

function createEmptyScope(ownerId = null, module = 'sales-manager') {
  return {
    loaded: false,
    loading: false,
    error: null,
    loadedAt: null,
    ownerId,
    module,
    modules: [],
    teamCustomers: [],
    teamReps: [],
    teamOrders: [],
    priorityOrders: [],
    summary: {
      customers: 0,
      reps: 0,
      orders: 0,
      pending: 0,
      reviewing: 0,
      assigned: 0,
      confirmed: 0,
      preparing: 0,
      dispatched: 0,
      shipped: 0,
      delivered: 0,
      collected: 0,
      returned: 0,
      cancelled: 0,
    },
  };
}

function resolveOwnerId(session = {}) {
  return normalizeId(
    session?.sales_rep_id
      || session?.rep_id
      || session?.id
      || '',
  );
}

function isModuleVisible(session = {}, module) {
  const type = normalizeUserType(session?.userType || session?.user_type || session?.role || null, null);
  if (!module) return false;
  if (module.key === 'sales') return ['sales_rep', 'sales_manager', 'admin', 'sales_operator', 'hybrid_operator'].includes(type) || canAccessCustomerManagement(session);
  if (module.key === 'customers') return canAccessCustomerManagement(session) || isOperationalRole(session);
  if (module.key === 'orders') return canAccessCustomerManagement(session) || isOperationalRole(session) || hasCapability(session, ['orders.view', 'orders.create', 'orders.manage']);
  if (module.key === 'reps') return ['sales_manager', 'admin', 'operations_manager', 'hybrid_operator'].includes(type) || hasCapability(session, ['sales_manager.manage_reps', 'sales_manager.access']);
  if (module.key === 'catalog' || module.key === 'companies' || module.key === 'workflow' || module.key === 'reports') {
    return isOperationalRole(session) || hasCapability(session, module.requiredCapabilities || []);
  }
  const caps = normalizeCapabilityList(session?.capabilities || session?.system_user?.capabilities || []);
  const required = Array.isArray(module?.requiredCapabilities) ? module.requiredCapabilities : [];
  if (!required.length) return false;
  return required.some((capability) => caps.includes(capability)) || isOperationalRole(session);
}

export function isOperationalModuleReady(moduleKey) {
  const module = getOperationalModuleByKey(moduleKey);
  return Boolean(module?.runtimeReady);
}

export function getOperationalModuleLabel(moduleKey) {
  const module = getOperationalModuleByKey(moduleKey);
  return module?.label || normalizeId(moduleKey) || 'وحدة تشغيلية';
}

export function getOperationalModules(session = {}) {
  return OPERATIONAL_MODULES
    .filter((module) => isModuleVisible(session, module))
    .map((module) => ({
      ...module,
      isReady: Boolean(module.runtimeReady),
      statusLabel: module.runtimeReady ? 'جاهز' : 'قريبًا',
      statusTone: module.runtimeReady ? 'success' : 'muted',
    }));
}

export function getOperationalModuleByKey(moduleKey) {
  const key = normalizeId(moduleKey);
  return OPERATIONAL_MODULES.find((module) => module.key === key) || null;
}

export function getDefaultOperationalModule(session = {}) {
  const type = normalizeUserType(session?.userType || session?.user_type || session?.role || null, null);
  const modules = getOperationalModules(session);
  const readyModules = modules.filter((module) => module.isReady);
  if (['sales_rep'].includes(type)) return 'sales';
  if (readyModules.some((module) => module.key === 'orders')) return 'orders';
  if (readyModules.some((module) => module.key === 'sales-manager')) return 'sales-manager';
  if (readyModules.some((module) => module.key === 'sales')) return 'sales';
  if (readyModules.length) return readyModules[0].key;
  return modules[0]?.key || 'sales-manager';
}

export function hasOperationalAccess(session = {}) {
  return isOperationalRole(session) || hasCapabilityOperationalAccess(session) || getOperationalModules(session).length > 0;
}

export function getOperationalRouteForModule(moduleKey = 'sales-manager') {
  const module = getOperationalModuleByKey(moduleKey);
  return module?.route || `ops/${normalizeId(moduleKey) || 'sales-manager'}`;
}

export function getOperationalQuickActions(session = {}) {
  const type = normalizeUserType(session?.userType || session?.user_type || session?.role || null, null);
  const actions = [
    { key: 'account', label: 'حسابي', icon: '👤', action: 'go-account', description: 'معلومات الجلسة والإعدادات', visible: true },
    { key: 'invoices', label: 'فواتيري', icon: '📦', action: 'go-invoices', description: 'الطلبات والفواتير السابقة', visible: true },
    { key: 'customers', label: 'عملائي', icon: '👥', action: 'go-customers', description: 'العملاء المرتبطون بالحساب', visible: canAccessCustomerManagement(session) },
    { key: 'checkout', label: 'إنشاء طلب', icon: '🛒', action: 'go-checkout', description: 'فتح مسار الطلب مباشرة', visible: ['sales_rep', 'sales_manager', 'admin'].includes(type) || canAccessCustomerManagement(session) },
    { key: 'orders', label: 'الطلبات', icon: '🧾', action: 'go-ops-module', module: 'orders', description: 'متابعة دورة الطلبات', visible: hasOperationalAccess(session) || canAccessOperationalDashboard(session) },
    { key: 'reps', label: 'المناديب', icon: '🧑‍💼', action: 'go-ops-module', module: 'reps', description: 'متابعة الفريق الميداني', visible: ['sales_manager', 'admin', 'operations_manager', 'hybrid_operator'].includes(type) },
    { key: 'catalog', label: 'الكتالوج', icon: '📚', action: 'go-ops-module', module: 'catalog', description: 'المنتجات والأسعار', visible: isOperationalRole(session) || hasCapability(session, ['products.manage', 'catalog.manage']) },
    { key: 'companies', label: 'الشركات', icon: '🏢', action: 'go-ops-module', module: 'companies', description: 'إدارة الشركات', visible: isOperationalRole(session) || hasCapability(session, ['companies.manage', 'company.manage']) },
    { key: 'reports', label: 'التقارير', icon: '📊', action: 'go-ops-module', module: 'reports', description: 'قراءة سريعة للأداء', visible: isOperationalRole(session) || hasCapability(session, ['reports.view']) },
  ];

  return actions.filter((action) => action.visible).map((action) => ({ ...action }));
}

function getWorkflowPriorityRank(stateKey) {
  const normalized = normalizeWorkflowStateKey(stateKey) || 'pending';
  return WORKFLOW_PRIORITY_RANK[normalized] || 99;
}

function buildPriorityOrders(orders = [], session = {}, limit = 6) {
  const list = Array.isArray(orders) ? orders : [];
  return list
    .map((order) => {
      const stateKey = normalizeWorkflowStateKey(order.workflow_state_key || order.workflow_status || order.status) || 'pending';
      const workflow = resolveWorkflowActions(order, session);
      return {
        ...order,
        workflowStateKey: stateKey,
        workflowLabel: getWorkflowStateLabel(stateKey),
        workflowPriority: getWorkflowPriorityRank(stateKey),
        workflowActions: workflow,
      };
    })
    .sort((left, right) => {
      const priorityDelta = Number(left.workflowPriority || 99) - Number(right.workflowPriority || 99);
      if (priorityDelta !== 0) return priorityDelta;
      const leftDate = new Date(left.created_at || 0).getTime();
      const rightDate = new Date(right.created_at || 0).getTime();
      if (leftDate !== rightDate) return rightDate - leftDate;
      return String(left.order_number || left.id).localeCompare(String(right.order_number || right.id));
    })
    .slice(0, Math.max(1, Number(limit || 6)))
    .map((order) => ({ ...order, workflowActions: { ...order.workflowActions } }));
}

async function loadManagerCustomers(api, ownerId, session = {}) {
  const rows = await api.get('customers', {
    select: '*',
    order: 'created_at.desc',
    limit: '100',
  }).catch(() => []);
  const list = Array.isArray(rows) ? rows : [];
  if (isGlobalOperationalScope(session)) return list;
  return list.filter((row) => matchesAnyOwner(row, ownerId, ['owner_user_id', 'sales_rep_id', 'created_by_rep_id', 'created_by']));
}

async function loadManagerReps(api, ownerId, session = {}) {
  if (!isGlobalOperationalScope(session) && !normalizeId(ownerId)) return [];
  const rows = await api.get('sales_reps', isGlobalOperationalScope(session) ? {
    select: 'id,name,phone,username,region,default_tier_name,is_active,is_blocked,blocked_reason,created_at,updated_at',
    order: 'created_at.desc',
    limit: '100',
  } : {
    select: 'id,name,phone,username,region,default_tier_name,is_active,is_blocked,blocked_reason,created_at,updated_at',
    id: `eq.${ownerId}`,
    order: 'created_at.desc',
  }).catch(() => []);
  return Array.isArray(rows) ? rows : [];
}

async function loadManagerOrders(api, ownerId, customerIds = [], session = {}) {
  const rows = await api.get('orders', {
    select: '*',
    order: 'created_at.desc',
    limit: '100',
  }).catch(() => []);
  const list = Array.isArray(rows) ? rows : [];
  if (isGlobalOperationalScope(session)) return list;
  const customerSet = new Set((Array.isArray(customerIds) ? customerIds : []).map((value) => normalizeId(value)).filter(Boolean));
  return list.filter((row) => {
    const ownerMatch = matchesAnyOwner(row, ownerId, ['sales_rep_id', 'rep_id', 'user_id', 'owner_user_id', 'created_by', 'created_by_rep_id']);
    const customerMatch = customerSet.size ? customerSet.has(normalizeId(row?.customer_id)) : false;
    return ownerMatch || customerMatch;
  });
}

function buildSummary(orders = [], customers = [], reps = []) {
  const summary = {
    customers: Array.isArray(customers) ? customers.length : 0,
    reps: Array.isArray(reps) ? reps.length : 0,
    orders: Array.isArray(orders) ? orders.length : 0,
    pending: 0,
    reviewing: 0,
    assigned: 0,
    preparing: 0,
    dispatched: 0,
    shipped: 0,
    delivered: 0,
    collected: 0,
    returned: 0,
    cancelled: 0,
  };

  for (const order of Array.isArray(orders) ? orders : []) {
    const key = String(order.workflow_state_key || order.workflow_status || order.status || '').trim();
    if (Object.prototype.hasOwnProperty.call(summary, key)) {
      summary[key] += 1;
    }
  }

  return summary;
}

let cachedScopeKey = null;
let cachedScope = createEmptyScope();

export async function loadManagerScope(api, session = {}, { force = false } = {}) {
  const ownerId = resolveOwnerId(session);
  const module = getDefaultOperationalModule(session);
  const globalScope = isGlobalOperationalScope(session);
  const scopeKey = `${ownerId || (globalScope ? 'global' : 'none')}:${module}`;

  if (cachedScope.loaded && cachedScopeKey === scopeKey && !force) {
    return {
      ...cachedScope,
      modules: cachedScope.modules.map((entry) => ({ ...entry })),
      teamCustomers: cachedScope.teamCustomers.map((entry) => ({ ...entry })),
      teamReps: cachedScope.teamReps.map((entry) => ({ ...entry })),
      teamOrders: cachedScope.teamOrders.map((entry) => ({ ...entry })),
      priorityOrders: cachedScope.priorityOrders.map((entry) => ({ ...entry, workflowActions: { ...entry.workflowActions } })),
      summary: { ...cachedScope.summary },
    };
  }

  const nextScope = createEmptyScope(ownerId || (globalScope ? 'global' : null), module);
  nextScope.loading = true;
  nextScope.modules = getOperationalModules(session);

  try {
    const teamCustomers = await loadManagerCustomers(api, ownerId, session);
    const customerIds = Array.from(new Set(teamCustomers.map((customer) => normalizeId(customer.id)).filter(Boolean)));
    const [teamReps, teamOrders] = await Promise.all([
      loadManagerReps(api, ownerId, session),
      loadManagerOrders(api, ownerId, customerIds, session),
    ]);

    nextScope.loaded = true;
    nextScope.loading = false;
    nextScope.error = null;
    nextScope.loadedAt = new Date().toISOString();
    nextScope.teamCustomers = teamCustomers;
    nextScope.teamReps = teamReps;
    nextScope.teamOrders = teamOrders;
    nextScope.priorityOrders = buildPriorityOrders(teamOrders, session, 8);
    nextScope.summary = buildSummary(teamOrders, teamCustomers, teamReps);

    cachedScopeKey = scopeKey;
    cachedScope = nextScope;

    publishDomainEvent('manager.scope.loaded', {
      owner_id: ownerId,
      module,
      customers: nextScope.summary.customers,
      reps: nextScope.summary.reps,
      orders: nextScope.summary.orders,
    });

    return {
      ...nextScope,
      modules: nextScope.modules.map((entry) => ({ ...entry })),
      teamCustomers: nextScope.teamCustomers.map((entry) => ({ ...entry })),
      teamReps: nextScope.teamReps.map((entry) => ({ ...entry })),
      teamOrders: nextScope.teamOrders.map((entry) => ({ ...entry })),
      priorityOrders: nextScope.priorityOrders.map((entry) => ({ ...entry, workflowActions: { ...entry.workflowActions } })),
      summary: { ...nextScope.summary },
    };
  } catch (error) {
    nextScope.loaded = true;
    nextScope.loading = false;
    nextScope.error = error?.message || 'MANAGER_SCOPE_LOAD_FAILED';
    nextScope.loadedAt = new Date().toISOString();
    cachedScopeKey = scopeKey;
    cachedScope = nextScope;
    return {
      ...nextScope,
      modules: nextScope.modules.map((entry) => ({ ...entry })),
      teamCustomers: [],
      teamReps: [],
      teamOrders: [],
      priorityOrders: [],
      summary: { ...nextScope.summary },
    };
  }
}

export async function loadManagerScopeIntoState(store, api, session = null, { force = false } = {}) {
  const current = store.getState();
  const scope = await loadManagerScope(api, session || current.auth.session || {}, { force });
  store.update((draft) => {
    draft.runtime.manager = {
      ...draft.runtime.manager,
      ...scope,
      modules: Array.isArray(scope.modules) ? scope.modules.map((entry) => ({ ...entry })) : [],
      teamCustomers: Array.isArray(scope.teamCustomers) ? scope.teamCustomers.map((entry) => ({ ...entry })) : [],
      teamReps: Array.isArray(scope.teamReps) ? scope.teamReps.map((entry) => ({ ...entry })) : [],
      teamOrders: Array.isArray(scope.teamOrders) ? scope.teamOrders.map((entry) => ({ ...entry })) : [],
      priorityOrders: Array.isArray(scope.priorityOrders) ? scope.priorityOrders.map((entry) => ({ ...entry, workflowActions: { ...entry.workflowActions } })) : [],
      summary: { ...(scope.summary || {}) },
      ownerId: scope.ownerId || null,
      module: scope.module || 'sales-manager',
    };
    draft.runtime.loading.manager = Boolean(scope.loading);
    draft.runtime.lifecycle.managerReady = Boolean(scope.loaded && !scope.loading && !scope.error);
  }, { dirty: ['opsNav', 'page', 'header', 'drawer', 'modals'] });
  return scope;
}

export function getManagerScopeSnapshot() {
  return {
    ...cachedScope,
    modules: cachedScope.modules.map((entry) => ({ ...entry })),
    teamCustomers: cachedScope.teamCustomers.map((entry) => ({ ...entry })),
    teamReps: cachedScope.teamReps.map((entry) => ({ ...entry })),
    teamOrders: cachedScope.teamOrders.map((entry) => ({ ...entry })),
    priorityOrders: cachedScope.priorityOrders.map((entry) => ({ ...entry, workflowActions: { ...entry.workflowActions } })),
    summary: { ...cachedScope.summary },
  };
}

export function getManagerWorkflowOverview(session, orders = []) {
  return buildPriorityOrders(orders, session, orders.length || 0);
}

export function createManagerRuntimeFacade() {
  return {
    getOperationalModules,
    getOperationalModuleByKey,
    getDefaultOperationalModule,
    getOperationalRouteForModule,
    getOperationalQuickActions,
    isOperationalModuleReady,
    getOperationalModuleLabel,
    hasOperationalAccess,
    loadManagerScope,
    loadManagerScopeIntoState,
    getManagerScopeSnapshot,
    getManagerWorkflowOverview,
  };
}
