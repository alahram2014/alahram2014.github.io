...
import { getOwnershipActorId, isSalesRepSession, normalizeSessionRecord, normalizeUserType } from './authService.js';
import { canViewCustomer } from './ownershipService.js';
import { publishDomainEvent } from './domainEventService.js';
import { canAddProductToCart } from '../runtime/modules/availabilityRuntime.js';
...
export function validateCheckout(state, tier, totals) {
  ...
  const productIndex = state.commerce?.catalog?.productIndex || {};
  for (const item of state.commerce.cart) {
    if (item.type !== 'product') continue;
    const product = productIndex[item.id] || productIndex[item.product_id];
    if (!product) return { ok: false, code: 'PRODUCT_MISSING', message: `المنتج ${item.title} غير متاح في الكتالوج` };
    if (!canAddProductToCart(product, item.unit || item.unit_code)) {
      return { ok: false, code: 'PRODUCT_UNAVAILABLE', message: `المنتج ${item.title} غير متاح للشراء حالياً` };
    }
  }
...
export async function submitOrder(api, state, tier, totals) {
  ...
  const isRepSession = isSalesRepSession(session);
  ...
  if (normalizedSession && !canViewCustomer(normalizedSession, customer)) {
  ...
  var ownerId = null;
  if (isRepSession) {
    salesRepId = getOwnershipActorId(session);
    ownerId = salesRepId;
  } else if (!isCustomerSession) {
    salesRepId = customer?.sales_rep_id || customer?.rep_id || null;
    ownerId = customer?.owner_id || salesRepId || null;
  }

  const locationPromise = captureOrderLocation();
  const productIndex = state.commerce?.catalog?.productIndex || {};
  for (const item of state.commerce.cart) {
    if (item.type !== 'product') continue;
    const product = productIndex[item.id] || productIndex[item.product_id];
    if (!product || !canAddProductToCart(product, item.unit || item.unit_code)) {
      throw new Error(`PRODUCT_UNAVAILABLE: ${item.title || item.product_id}`);
    }
  }
  const items = state.commerce.cart.map((item) => normalizeOrderItem(item, tier));

const normalizedUserType = isRepSession ? 'rep' : 'customer';
  ...
  if (location) {
    await api.patch('orders', {
      ...
      location_maps_url: location.mapsUrl,
    }, { id: 'eq.' + order.id }).catch(function() {});
    order.location_lat = location.lat;
    order.location_lng = location.lng;
    order.location_maps_url = location.mapsUrl;
  }
  ...
  return { order: order, items: normalizedItems, customer: customer };
}
