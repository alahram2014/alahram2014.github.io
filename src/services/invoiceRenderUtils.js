...
export function renderInvoiceLocation(invoice) {
  if (!invoice.location_maps_url) return '';
  var lat = invoice.location_lat;
  var lng = invoice.location_lng;
  var url = invoice.location_maps_url;
  return '<div class="invoice-detail__location">' +
    '<span class="location-icon">📍</span>' +
    '<strong>موقع التسليم</strong>' +
    '<a href="' + dom.escape(url) + '" target="_blank" rel="noopener" class="invoice-detail__location-link">' +
    dom.escape(String(lat)) + ', ' + dom.escape(String(lng)) +
    '</a>' +
    '</div>';
}

export function renderInvoiceActions(invoiceDataEncoded, _isPending = false, _invoiceId = '') {
  return `<div class="invoice-detail__actions">
    ...
    <button class="btn btn--primary" type="button" data-action="resend-whatsapp" data-invoice-data="${dom.escape(invoiceDataEncoded)}">إعادة إرسال واتساب</button>
