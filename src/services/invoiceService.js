...
function renderSenderSection({ session, customer }) {
  ...
  const hasRepAssignment = Boolean(customer?.owner_id || customer?.sales_rep_id || session?.sales_rep_id || session?.rep_id);
...
export function buildWhatsAppInvoice({ order, items, session, customer, tierLabel, supportWhatsapp }) {
  ...
  const locationText = order.location_maps_url
    ? `📍 ${buyer.name || 'العميل'}\n${order.location_maps_url}`
    : `لوكيشن العميل:\n${resolveCustomerLocation(buyer)}`;
