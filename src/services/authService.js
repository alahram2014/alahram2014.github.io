...
function collectSessionCapabilities(session) {
  const buckets = [
    session?.capabilities,
    session?.permissions,
    session?.system_user?.capabilities,
    session?.system_user?.permissions,
  ];
  return mergeUniqueStrings(...buckets);
}

export function normalizeCapabilityList(...lists) {
  return mergeUniqueStrings(...lists);
}

export function hasCapability(session, required = []) {
  const capabilities = collectSessionCapabilities(session);
  const requiredList = Array.isArray(required) ? required : [required];
  return requiredList.some((value) => capabilities.includes(normalizeIdentifier(value)));
}
...
export function hasOperationalAccess(session = {}) {
  ...
  if (type && OPERATIONAL_ROLES.has(type)) return true;
  return hasCapability(normalized, OPERATIONAL_CAPABILITIES);
}
...
export function normalizeSessionRecord(session) {
  ...
  const capabilities = mergeUniqueStrings(session.capabilities, session.permissions, session.system_user?.capabilities, session.system_user?.permissions);
  const domains = mergeUniqueStrings(session.domains, session.system_user?.domains);
  const systemUser = session.system_user && typeof session.system_user === 'object'
    ? {
        ...session.system_user,
        capabilities: mergeUniqueStrings(session.system_user.capabilities, session.system_user.permissions),
        permissions: mergeUniqueStrings(session.system_user.permissions),
        domains: mergeUniqueStrings(session.system_user.domains),
      }
    : session.system_user ?? null;
  ...
  return {
    ...
    capabilities,
    permissions: mergeUniqueStrings(session.permissions),
    domains,
...
export function canAccessOperationalDashboard(session = {}) {
  return hasOperationalAccess(session) || hasCapability(session, ['dashboard.admin', 'system.manage_users', 'system.manage_capabilities']);
}
