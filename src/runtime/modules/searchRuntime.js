...
function renderAiSearchResults(el, results, q) {
  ...
  const productHtml = products.length ? '<div class="header-search__group-label">المنتجات</div>' + products.map((p) => {
    const label = p._availabilityLabel || '';
    const badgeClass = p._availabilityBadgeClass || '';
    const metaParts = [p.company_name || 'منتج'];
    if (label) metaParts.push('<span class="availability-badge ' + dom.escape(badgeClass) + '">' + dom.escape(label) + '</span>');
    return '<div class="header-search__result-item" data-action="global-search-select" data-product-id="' + dom.escape(String(p.id)) + '"><span class="header-search__result-thumb"></span><span class="header-search__result-name">' + dom.escape(p.product_name || '—') + '</span><span class="header-search__result-meta">' + metaParts.join(' ') + '</span></div>';
  }).join('') : '';
