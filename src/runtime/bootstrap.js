...
import { validateCheckout, submitOrder, saveEditedInvoice, restoreInvoiceToCart, deleteOrder, isOrderEditable } from '../services/orderService.js';
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'submit-checkout') {
      ...
      if (editingId) {
        ...
        try {
          ...
          const result = await saveEditedInvoice(api, editingId, state, tier, totals);
          clearCart();
          store.patch({ ui: { ...store.getState().ui, editingInvoiceId: null }, commerce: { ...store.getState().commerce, cart: [] } });
          notify(store, 'success', 'تم حفظ التعديلات', 'فاتورة ' + String(result.invoiceId).slice(0, 8));
          loadInvoicesIntoState(store, api);
          navigateAuthority(store, 'invoice', { invoiceId: result.invoiceId });
        } catch (e) {
          notify(store, 'error', 'تعذر حفظ التعديلات', e?.details || e.message || '');
        } finally {
    ...
    if (action === 'edit-invoice') {
      ...
      if (!invoiceId) return;
      const state = store.getState();
      if (state.ui.editingInvoiceId) {
        notify(store, 'warning', 'يوجد تعديل قيد التنفيذ', 'أنهِ التعديل الحالي أولًا');
        return;
      }
      ...
      if (!invoice || !isOrderEditable(invoice) || (session && !canViewInvoice(session, invoice))) {
        notify(store, 'error', 'غير مصرح', 'لا يمكن تعديل هذه الفاتورة');
        return;
      }
      store.update(function(draft) { draft.ui.editingInvoiceId = invoiceId; draft.commerce.cart = []; }, { silent: true });
      restoreInvoiceToCart(api, invoiceId).then(function(cartItems) {
        store.update(function(draft) {
          draft.commerce.cart = cartItems;
          var cid = invoice.customer_id || '';
          var allCustomers = draft.commerce.customers || [];
          var customer = allCustomers.find(function(c) { return String(c.id) === String(cid); }) || { id: cid, name: invoice.customer_name_snapshot || invoice.customer_name || '' };
          draft.auth.selectedCustomer = customer;
          draft.ui.editingInvoiceId = invoiceId;
        }, { dirty: ['page', 'header'] });
        navigateAuthority(store, 'checkout');
      }).catch(function(e) {
        store.patch({ ui: { ...store.getState().ui, editingInvoiceId: null } });
        notify(store, 'error', 'تعذر تحميل الفاتورة', e?.details || '');
      });
      return;
    }

    if (action === 'cancel-edit-invoice') {
      store.patch({ ui: { ...store.getState().ui, editingInvoiceId: null } });
      clearCart();
      store.patch({ commerce: { ...store.getState().commerce, cart: [] } });
      schedule('page');
    ...
    if (action === 'delete-invoice') {
      ...
      const invoice = (state.commerce.invoices || []).find(function(inv) { return String(inv.id) === String(invoiceId); });
      if (!invoice || !isOrderEditable(invoice) || (session && !canViewInvoice(session, invoice))) {
        notify(store, 'error', 'غير مصرح', 'لا يمكن حذف هذه الفاتورة');
        return;
      }
      deleteOrder(api, invoiceId).then(function() {
        notify(store, 'success', 'تم حذف الفاتورة', '');
        loadInvoicesIntoState(store, api);
        if (store.getState().app.route.name === 'invoice' || store.getState().app.route.params.invoiceId === invoiceId) navigateAuthority(store, 'invoices');
        schedule('page');
      }).catch(function(e) {
        notify(store, 'error', 'تعذر حذف الفاتورة', e?.details || '');
      });
      return;
    }
  ...
  document.addEventListener('change', (event) => {
    const target = event.target;
    if (!(target instanceof HTMLInputElement)) return;
    if (target.getAttribute('data-role') === 'invoice-edit-qty') {
      const itemId = target.getAttribute('data-item-id');
      const invoiceId = target.getAttribute('data-invoice-id');
      if (!itemId || !invoiceId) return;
      const qty = parseInt(target.value, 10);
      if (!qty || qty < 1) { target.value = '1'; return; }
      updateOrderItemQty(api, itemId, qty).then(() => {
        updateOrderTotals(api, invoiceId).catch(() => {});
      }).catch((e) => {
        notify(store, 'error', 'تعذر تحديث الكمية', e?.details || '');
      });
      return;
    }
  });
...
export async function bootstrapApp() {
  ...
  function renderGlobalSearchResults(store, nodes) {
    ...
    const session = normalizeSessionRecord(state.auth.session);
    void performAiSearch(q, session, state).then((results) => {
