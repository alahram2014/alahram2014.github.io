...
import { hydrateOpsRuntime } from '../ops/hydration/runtimeHydration.js';
...
export async function bootstrapApp() {
  ...
  window.addEventListener('hashchange', () => {
    ...
    if (nextRoute.name === 'ops') {
      void hydrateOpsRuntime(store, api, nextRoute);
    }
  });
  ...
  if (initialRoute.name === 'ops') {
    void hydrateOpsRuntime(store, api, initialRoute);
  }
  store.update((draft) => {
      ...
      try {
        ...
        if (projectedSession) {
          ...
          if (userChanged) {
            ...
            if (hasOperationalAccess(nextSession)) {
