...
import { login, logout, registerCustomer, normalizeUserType, normalizeSessionRecord, getOwnershipActorId, hasCapability, persistSessionRecord, readPersistedSession, canAccessCustomerManagement, canAccessOperationalDashboard, refreshSessionProjection, isSalesRepSession } from '../services/authService.js';
import { loadRepCustomers, createCustomer, persistSelectedCustomer } from '../services/customerService.js';
import { hasOperationalAccess, getDefaultOperationalModule, isOperationalModuleReady } from '../services/managerService.js';
...
import { ensureOpsRouteHydrated, runOpsCatalogAction, runOpsCustomerAction, runOpsRepAction, runOpsOrderAction } from '../ops/runtime/routeHydration.js';
...
function getOpsRouteSection(route) {
  if (!route || route.name !== 'ops') return 'dashboard';
  return String(route.params?.section || 'dashboard').trim() || 'dashboard';
}
...
function renderPage(state, nodes) {
  ...
  switch (route.name) {
    ...
    case 'ops':
    case 'sales-manager': pageHtml = renderOpsRoute(state); break;
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    const refreshOpsRoute = async () => {
      const currentRoute = store.getState().app.route;
      if (currentRoute?.name === 'ops') {
        await ensureOpsRouteHydrated(store, api, currentRoute);
        schedule('page', 'header', 'opsNav', 'toast');
      }
    };
    ...
    if (action === 'go-ops') {
      ...
      return navigateAuthority(store, 'ops', { section: getDefaultOperationalModule(state.auth.session) });
    }
    if (action === 'go-ops-module') {
      ...
      return navigateAuthority(store, 'ops', { section: module });
    }
    if (action === 'go-ops-route') {
      const section = String(target.getAttribute('data-ops-section') || '').trim() || 'dashboard';
      const id = String(target.getAttribute('data-ops-id') || '').trim();
      if (!canOpenOpsWorkspace(state.auth.session)) {
        notify(store, 'warning', 'غير مصرح', 'هذه اللوحة متاحة للحسابات التشغيلية فقط');
        return;
      }
      return navigateAuthority(store, 'ops', id ? { section, id } : { section });
    }
    if (action === 'ops-catalog-quick-edit' || action === 'ops-catalog-inventory' || action === 'ops-catalog-pricing' || action === 'ops-catalog-visibility' || action === 'ops-catalog-stock') {
      const productId = String(target.getAttribute('data-product-id') || '').trim();
      const currentProduct = (store.getState().runtime?.ops?.catalog?.products || []).find((item) => String(item.id || '') === productId);
      if (!productId) return;
      try {
        if (action === 'ops-catalog-quick-edit') {
          const name = window.prompt('اسم المنتج', currentProduct?.product_name || '');
          if (name === null) return;
          await runOpsCatalogAction(api, 'quick-edit', productId, { product_name: name.trim() });
        } else if (action === 'ops-catalog-inventory' || action === 'ops-catalog-stock') {
          const nextQty = window.prompt('الكمية الحالية', String(currentProduct?.stock_quantity ?? 0));
          if (nextQty === null) return;
          await runOpsCatalogAction(api, 'inventory-update', productId, Number(nextQty));
        } else if (action === 'ops-catalog-pricing') {
          const nextPrice = window.prompt('السعر الجديد', String(currentProduct?.unit_price ?? currentProduct?.base_price ?? 0));
          if (nextPrice === null) return;
          await runOpsCatalogAction(api, 'pricing-update', productId, Number(nextPrice));
        } else if (action === 'ops-catalog-visibility') {
          await runOpsCatalogAction(api, 'visibility-toggle', productId, !(currentProduct?.active && !currentProduct?.hidden));
        }
        await refreshOpsRoute();
      } catch (error) {
        console.error(error);
        notify(store, 'error', 'تعذر تنفيذ العملية', error?.message || '');
      }
      return;
    }
    if (action === 'ops-order-approve' || action === 'ops-order-assign' || action === 'ops-order-confirm' || action === 'ops-order-prepare' || action === 'ops-order-ship' || action === 'ops-order-deliver' || action === 'ops-order-cancel' || action === 'ops-order-return') {
      const orderId = String(target.getAttribute('data-order-id') || '').trim();
      if (!orderId) return;
      try {
        await runOpsOrderAction(api, action.replace('ops-order-', ''), orderId);
        await refreshOpsRoute();
      } catch (error) {
        console.error(error);
        notify(store, 'error', 'تعذر تحديث الطلب', error?.message || '');
      }
      return;
    }
    if (action === 'ops-customer-quick-edit' || action === 'ops-customer-assign-rep' || action === 'ops-customer-toggle' || action === 'ops-customer-notes') {
      const customerId = String(target.getAttribute('data-customer-id') || '').trim();
      const currentCustomer = (store.getState().runtime?.ops?.customers?.customers || []).find((item) => String(item.id || '') === customerId);
      if (!customerId) return;
      try {
        if (action === 'ops-customer-assign-rep') {
          const repId = window.prompt('رقم المندوب', currentCustomer?.sales_rep_id || '');
          if (repId === null) return;
          await runOpsCustomerAction(api, 'assign-rep', customerId, repId.trim());
        } else if (action === 'ops-customer-toggle') {
          await runOpsCustomerAction(api, 'activate-toggle', customerId, !(currentCustomer?.active && !currentCustomer?.blocked));
        } else if (action === 'ops-customer-notes') {
          const notes = window.prompt('ملاحظات تشغيلية', currentCustomer?.notes || '');
          if (notes === null) return;
          await runOpsCustomerAction(api, 'operational-notes', customerId, notes);
        } else if (action === 'ops-customer-quick-edit') {
          const name = window.prompt('اسم العميل', currentCustomer?.name || '');
          if (name === null) return;
          await runOpsCustomerAction(api, 'quick-edit', customerId, { name: name.trim() });
        }
        await refreshOpsRoute();
      } catch (error) {
        console.error(error);
        notify(store, 'error', 'تعذر تحديث العميل', error?.message || '');
      }
      return;
    }
    if (action === 'ops-rep-assign-customers' || action === 'ops-rep-toggle' || action === 'ops-rep-visibility') {
      const repId = String(target.getAttribute('data-rep-id') || '').trim();
      const currentRep = (store.getState().runtime?.ops?.reps?.reps || []).find((item) => String(item.id || '') === repId);
      if (!repId) return;
      try {
        if (action === 'ops-rep-assign-customers') {
          const customerIds = window.prompt('أدخل العملاء مفصولين بفواصل', '');
          if (customerIds === null) return;
          await runOpsRepAction(api, 'assign-customers', repId, customerIds.split(',').map((value) => value.trim()).filter(Boolean));
        } else if (action === 'ops-rep-toggle') {
          await runOpsRepAction(api, 'activate-toggle', repId, !(currentRep?.active && !currentRep?.blocked));
        } else if (action === 'ops-rep-visibility') {
          const notes = window.prompt('الرؤية التشغيلية', currentRep?.notes || '');
          if (notes === null) return;
          await runOpsRepAction(api, 'operational-visibility', repId, { notes: notes.trim() });
        }
        await refreshOpsRoute();
      } catch (error) {
        console.error(error);
        notify(store, 'error', 'تعذر تحديث المندوب', error?.message || '');
      }
      return;
    }
    ...
    if (action === 'workflow-transition') {
      ...
      if (!store.getState().runtime.workflow.loaded && !store.getState().runtime.workflow.loading) {
        try {
          await loadWorkflowRuntime(api);
        } catch (error) {
          console.error(error);
        }
      }
...
export async function bootstrapApp() {
  ...
  const scheduler = createRenderLoop({
    ...
    floating: () => { if (!isRuntimeInteractive(store.getState())) return; nodes.floating.innerHTML = renderFloatingExecutionBar(store.getState()); },
  ...
  function renderContent() {
    ...
    else if (route === 'ops' || route === 'sales-manager') html = renderOpsRoute(state);
  ...
  function applyBodyFlags() {
    ...
    nodes.floating.classList.toggle('is-hidden', operationalRoute || checkoutRoute || ['login', 'register', 'invoice'].includes(route));
  ...
  window.addEventListener('hashchange', () => {
    ...
    if (nextRoute.name === 'ops') {
      void ensureOpsRouteHydrated(store, api, nextRoute);
    }
  });
  ...
  if (initialRoute.name === 'ops') {
    void ensureOpsRouteHydrated(store, api, initialRoute);
  }
  store.update((draft) => {
  ...
  void (async () => {
      ...
      try {
        ...
        if (projectedSession) {
          ...
          if (userChanged) {
            ...
            scheduler.schedule('header', 'opsNav', 'page', 'drawer', 'modals');
          }
        }
      } catch (error) {
        console.error(error);
    ...
    if (initialRoute.name !== 'ops') {
      ...
      summary = { companies: [], products: [], productIndex: {}, offers: { daily: [], flash: [] }, tiers: [], settings: [], settingsMap: {}, top: { products: [], companies: [] }, counters: { companies: 0, tiers: 0, deals: 0, flash: 0 }, catalogProducts: [] };
      selectedTier = normalizeTierName(selectedTier) || 'base';
      store.update((draft) => {
        draft.commerce.catalog = { ...draft.commerce.catalog, ...summary };
        draft.commerce.selectedTier = selectedTier;
        draft.commerce.priceBook = { tierName: selectedTier, products: {} };
        draft.runtime.loading.catalog = false;
        draft.runtime.loading.pricing = false;
        draft.runtime.lifecycle.catalogReady = false;
        draft.runtime.lifecycle.offersReady = false;
        draft.runtime.lifecycle.flashOffersReady = false;
        draft.runtime.lifecycle.companiesReady = false;
        draft.runtime.lifecycle.pricingReady = false;
      }, { dirty: ['header', 'banner', 'page', 'hero', 'footer', 'search'] });
    }
