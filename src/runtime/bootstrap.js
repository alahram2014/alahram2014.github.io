...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'workflow-transition') {
      ...
      const selectValue = form?.querySelector?.('[data-role="workflow-next-state"]')?.value;
      const nextStateKey = String(target.getAttribute('data-next-state-key') || selectValue || '').trim();
  ...
  document.addEventListener('input', (event) => {
    ...
    if (target.getAttribute('data-role') === 'ops-search') {
      store.patch({ ui: { ...state.ui, search: target.value } }, { silent: true });
      clearTimeout(searchTypingTimer);
      searchTypingTimer = setTimeout(() => schedule('page', 'opsNav'), SEARCH_DEBOUNCE_MS);
      return;
    }
