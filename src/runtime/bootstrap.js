...
import { loadOpsProducts, createOpsProduct, updateOpsProduct, deleteOpsProduct, toggleOpsProductActive, toggleOpsProductVisibility } from '../services/productOpsService.js';
...
function bootstrapShell(root, routeName) {
  root.innerHTML = routeName === 'admin' ? adminShellTemplate() : shellTemplate();
}
...
function renderPage(state, nodes) {
  ...
  switch (route.name) {
    ...
    case 'rep': pageHtml = renderRepDashboardPage(state); break;
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'ops-product-edit') {
      ...
      document.getElementById('opsProductId').value = target.getAttribute('data-product-id') || '';
...
export async function bootstrapApp() {
  ...
  const initialRouteName = store.getState().app.route.name;
  ...
  window.addEventListener('hashchange', () => {
    ...
    const shellChanged = (prevRoute.name === 'admin') !== (nextRoute.name === 'admin');
    store.patch({
    ...
    scheduler.schedule('page', 'header', 'opsNav', 'adminHeader', 'adminSidebar', 'adminPage');
  });
  ...
  if (bootRoute.name === 'admin') {
    const appEl = document.getElementById('app');
    if (appEl) { appEl.innerHTML = ''; bootstrapShell(appEl, 'admin'); Object.assign(nodes, getNodes()); }
  }
