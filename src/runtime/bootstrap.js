...
import { buildPriceBook, persistSelectedTier, syncCartPrices, normalizeTierName } from '../services/pricingService.js';
import { addProductToCart, clearCart, hydrateCart, persistCart, recalcCart, removeItem, toggleOfferInCart, updateQty } from '../services/cartService.js';
...
import { startVisit, endVisit, findActiveVisit, loadRepVisits, getVisitTimeline, updateActiveVisitOrder, loadTeamVisits } from '../services/visitService.js';
import { renderVisitTimeline, renderEndVisitButton } from '../services/visitRenderUtils.js';
import { appendBehaviorEvent } from '../services/analyticsService.js';
import { bootstrapShell, getNodes, syncBodyShellHeight, isOperationalRoute, captureSearchFocus, restoreSearchFocus } from './modules/shellRuntime.js';
import { scheduleHydration, HYDRATION_PRIORITY } from './modules/hydrationRuntime.js';
import { SEARCH_DEBOUNCE_MS, cancelSearch, scheduleSearch, clearSearchResults, renderGlobalSearchResults } from './modules/searchRuntime.js';
import { performAppUpdate } from './modules/updateRuntime.js';
import { performInstall, hideInstallOverlay } from './modules/installRuntime.js';
...
import { renderHeader, patchAuthVisualState } from '../layout/header.js';
...
import { loadAuthorizedCustomers, loadAuthorizedInvoices, canViewCustomer, canViewInvoice } from '../services/ownershipService.js';
...
import { loadOpsProducts, createOpsProduct, updateOpsProduct, deleteOpsProduct, toggleOpsProductActive, toggleOpsProductVisibility, updateOpsProductUnitStock, updateOpsProductUnitPrice, addProductUnit, setOpsActorId } from '../services/productOpsService.js';
import { storageKeys, removeValue, purgeLegacyStorage, loadJSON, saveJSON } from '../core/storage.js';
import { closeTransientSurfaces, setPendingFlow, clearPendingFlow, navigateAuthority, resetCustomerLocationDraft, commitCustomerLocationDraft } from './modules/navigationRuntime.js';
import { setCheckoutBusy } from './modules/checkoutRuntime.js';
...
function ensureDesktopCSS() {
  ...
  link.rel = 'stylesheet';
  link.href = 'src/styles/desktop.css';
...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'select-customer') {
      ...
      const repCustomers = Array.isArray(store.getState().runtime.rep.customers) ? store.getState().runtime.rep.customers : [];
      let customer = repCustomers.find((c) => String(c.id) === String(customerId));
      if (!customer) {
        const allCustomers = Array.isArray(store.getState().commerce.customers) ? store.getState().commerce.customers : [];
        customer = allCustomers.find((c) => String(c.id) === String(customerId));
      }
      if (!customer || (session && !canViewCustomer(session, customer))) return;
      store.update((draft) => { draft.auth.selectedCustomer = customer; }, { action: 'customer-select', dirty: ['page', 'header', 'modals'] });
    ...
    if (action === 'ops-company-toggle-visibility') {
      ...
      if (!companyId) return;
      try {
        await api.post('rpc/toggle_company_visibility', { p_company_id: companyId });
    ...
    if (action === 'ops-product-search') {
      store.patch({ ui: { ...store.getState().ui, opsProductSearch: target.value } });
      schedule('page');
      return;
    }

    if (action === 'ops-product-edit-page') {
      const productId = target.getAttribute('data-product-id');
      if (!productId) return;
      store.patch({ ui: { ...state.ui, opsProductEditId: productId } });
      schedule('page');
      return;
    }

    if (action === 'ops-product-create-page') {
      store.patch({ ui: { ...state.ui, opsProductEditId: '__new__' } });
      schedule('page');
      return;
    }

    if (action === 'ops-product-back') {
      store.patch({ ui: { ...state.ui, opsProductEditId: null, opsProductSearch: '' } });
      void loadOpsProductsIntoState(store, api);
      schedule('page');
      return;
    }

    if (action === 'ops-product-add-unit') {
      const productId = target.getAttribute('data-product-id');
      const unitCode = document.getElementById('opsNewUnitCode')?.value;
      const stock = parseInt(document.getElementById('opsNewUnitStock')?.value || '0', 10);
      if (!productId || !unitCode) { notify(store, 'warning', 'بيانات الوحدة غير مكتملة', ''); return; }
      const tierInputs = document.querySelectorAll('[id^="opsNewUnitPrice_"]');
      let added = 0;
      for (const input of tierInputs) {
        ...
        try {
          await addProductUnit(api, productId, unitCode, tier, price, stock);
          added++;
        } catch { /* individual tier may fail */ }
      }
      if (added) {
        notify(store, 'success', 'تمت إضافة الوحدة', `${unitCode} مع ${added} شريحة سعرية`);
        store.update((draft) => { draft.ui.opsProductEditId = productId; }, { silent: true });
        void loadOpsProductsIntoState(store, api).then(() => schedule('page'));
      } else {
        notify(store, 'warning', 'تعذر إضافة الوحدة', '');
      }
      return;
    }
    ...
    if (action === 'ops-product-image-preview') {
      ...
      const container = target.closest('.ops-edit-col') || target.closest('.ops-field') || target.closest('.ops-modal__grid');
      if (!container) return;
      const existingPreview = document.getElementById('opsImagePreview');
      if (existingPreview) {
        existingPreview.style.display = url ? 'flex' : 'none';
        if (url) document.getElementById('opsImagePreviewImg').src = url;
      } else {
        let preview = container.querySelector('.ops-image-preview');
        if (url) {
          if (!preview) { preview = document.createElement('div'); preview.className = 'ops-image-preview'; target.parentNode.insertBefore(preview, target.nextSibling); }
          preview.innerHTML = `<img src="${dom.escape(url)}" alt="معاينة" style="max-width:120px;max-height:120px;border-radius:8px">`;
    ...
    if (action === 'pwa-install') {
      ...
      store.patch({ ui: { ...state.ui, accountMenuOpen: false } });
      schedule('header');
      var installResult = await performInstall(store);
      if (installResult === 'accepted') notify(store, 'success', 'تم تثبيت التطبيق', 'يمكنك الآن استخدامه كتطبيق مستقل');
      else if (installResult === 'dismissed') notify(store, 'info', 'تم إلغاء التثبيت', '');
      else if (installResult === 'installed') notify(store, 'info', 'التطبيق مثبت بالفعل', 'يمكنك استخدامه من الشاشة الرئيسية أو المتصفح');
      else if (installResult === 'unsupported') notify(store, 'info', 'تثبيت التطبيق', 'المتصفح لا يدعم التثبيت المباشر');
      else if (installResult === 'no-prompt') notify(store, 'info', 'تثبيت التطبيق', 'استخدم قائمة المتصفح أو افتح التطبيق من الشاشة الرئيسية');
      else if (installResult === 'error') notify(store, 'warning', 'تعذر تثبيت التطبيق', 'حاول مرة أخرى من قائمة المتصفح');
      return;
    }

    if (action === 'close-install-guide') { hideInstallOverlay(); return; }
    ...
    if (action === 'update-app') { closeTransientSurfaces(store, { keepDrawer: false }); store.patch({ ui: { ...state.ui, accountMenuOpen: false } }); schedule('header'); performAppUpdate(); return; }
    if (action === 'set-theme') {
      ...
      setTheme(nextTheme);
      schedule('theme', 'header', 'banner', 'search', 'hero', 'page', 'footer');
      return;
    }
    ...
    if (action === 'close-modal') { closeTransientSurfaces(store); resetCustomerLocationDraft(store); schedule('modals'); return; }
    ...
    if (action === 'toggle-product') {
      ...
      if (result.reason === 'PRODUCT_UNAVAILABLE') {
        notify(store, 'warning', 'المنتج غير متاح', 'لا يمكن إضافة منتج غير متوفر أو منتهي الكمية');
        return;
      }
      store.update((draft) => {
    ...
    if (action === 'submit-checkout') {
      const state = store.getState();
      const validation = validateCheckout(state, getSelectedTier(state), computeCartTotals(state));
      if (!validation.ok) {
    ...
    if (action === 'view-invoice') {
      ...
      if (!invoice || (session && !canViewInvoice(session, invoice))) return;
      let items = state.commerce.invoiceItemsById?.[String(invoiceId)] || [];
      if (!items.length) {
        items = await api.get('order_items', {
          select: 'id,order_id,product_id,type,qty,price,unit,created_at,product_name_snapshot,company_id_snapshot,company_name_snapshot,unit_name_snapshot',
          order_id: `eq.${invoiceId}`,
          order: 'created_at.asc',
        }).catch(() => []);
        items = Array.isArray(items) ? items : [];
        const names = new Map(Object.values(state.commerce.catalog.productIndex || {}).map((row) => [String(row.product_id), row.product_name || '']));
        items = items.map((item) => ({
          ...item,
          title: item.product_name_snapshot || item.title || names.get(String(item.product_id)) || item.product_id,
        }));
      }
      store.update((draft) => {
    ...
    if (action === 'whatsapp-invoice') {
      const invoiceId = target.getAttribute('data-invoice-id');
      const whatsapp = target.getAttribute('data-whatsapp');
      if (!invoiceId || !whatsapp) return;
      const state = store.getState();
      const session = normalizeSessionRecord(state.auth.session);
      const invoice = (state.commerce.invoices || []).find((item) => String(item.id) === String(invoiceId));
      if (!invoice || (session && !canViewInvoice(session, invoice))) return;
      ...
      const msg = buildWhatsAppInvoice({ order: invoice, items, session: state.auth.session, customer: invoice, tierLabel: '', supportWhatsapp: whatsapp });
      window.open(msg, '_blank');
      return;
    }

    if (action === 'resend-whatsapp') {
      const encoded = target.getAttribute('data-invoice-data');
      if (!encoded) return;
      try {
        const data = JSON.parse(decodeURIComponent(encoded));
        const companies = store.getState().commerce.companies || [];
        generateInvoicePDF(data.invoice, data.groupedItems, companies);
        const supportWhatsapp = api.config.supportWhatsapp;
        if (supportWhatsapp) {
          const msg = buildWhatsAppInvoice({
            order: data.invoice,
            items: data.items,
        ...
        notify(store, 'success', 'تم إرسال الفاتورة', 'تم إنشاء PDF وفتح واتساب');
      } catch (e) {
        notify(store, 'error', 'تعذر إرسال الفاتورة', e?.message || '');
      }
      return;
    }
    ...
    if (action === 'download-invoice-pdf') {
      ...
      try {
        ...
        if (typeof navigator !== 'undefined' && navigator.share) {
          navigator.share({
            title: 'فاتورة #' + (data.invoice.order_number || data.invoice.invoice_number || data.invoice.id),
            text: 'فاتورة شراء من الأهرام للتجارة والتوزيع',
            url: data.invoice.location_maps_url || window.location.href,
          }).catch(function() {
            generateInvoicePDF(data.invoice, data.groupedItems, companies);
          });
        } else {
        ...
        notify(store, 'error', 'تعذر إنشاء PDF', e?.message || '');
      }
      return;
    }
    ...
    if (action === 'start-visit') {
      ...
      if (!customerId) { notify(store, 'warning', 'خطأ', 'معرف العميل مطلوب'); return; }
      var session = store.getState().auth.session;
      if (!isSalesRepSession(session)) { notify(store, 'warning', 'غير مصرح', 'هذه الميزة للمندوبين فقط'); return; }
      var repId = getOwnershipActorId(session) || session.id;
      var repState = store.getState().runtime.rep;
      startVisit(api, repId, customerId, repState.customers).then(function(visit) {
        notify(store, 'success', 'تم بدء الزيارة', '');
        store.update(function(draft) {
          var visits = Array.isArray(repState.visits) ? repState.visits.slice() : [];
          visits.unshift(visit);
          draft.runtime.rep.visits = visits;
          draft.runtime.rep.activeVisit = visit;
        }, { dirty: ['page'] });
        schedule('page');
      }).catch(function(err) {
        notify(store, 'error', 'تعذر بدء الزيارة', err?.message || '');
      });
      return;
    }
    ...
    if (action === 'end-visit') {
      ...
      var endStatus = target.getAttribute('data-end-status') || 'completed';
      if (!visitId) { notify(store, 'warning', 'خطأ', 'معرف الزيارة مطلوب'); return; }
      var notes = target.getAttribute('data-notes') || null;
      var repState = store.getState().runtime.rep;
      var visits = Array.isArray(repState.visits) ? repState.visits.slice() : [];
      var activeVisit = repState.activeVisit;
      var customer = null;
      if (activeVisit) {
        customer = (Array.isArray(repState.customers) ? repState.customers : []).find(function(c) { return String(c.id) === String(activeVisit.customer_id); }) || null;
      }
      endVisit(api, visitId, endStatus, notes, null, visits, customer).then(function(updatedVisit) {
        var endLabel = endStatus === 'customer_closed' ? 'العميل مغلق' : endStatus === 'follow_up_required' ? 'متابعة' : 'تم إنهاء الزيارة';
        notify(store, 'success', endLabel, '');
        var updatedVisits = visits.slice();
        for (var vi = 0; vi < updatedVisits.length; vi++) {
          if (String(updatedVisits[vi].id) === String(visitId)) {
            updatedVisits[vi].visit_status = endStatus;
            updatedVisits[vi].ended_at = new Date().toISOString();
            if (updatedVisit) {
              updatedVisits[vi].visit_confidence_score = updatedVisit.visit_confidence_score;
              updatedVisits[vi].visit_flags = updatedVisit.visit_flags;
              updatedVisits[vi].duration_minutes = updatedVisit.duration_minutes;
            }
            break;
          }
        }
        store.update(function(draft) {
          draft.runtime.rep.visits = updatedVisits;
          draft.runtime.rep.activeVisit = null;
        }, { dirty: ['page'] });
        schedule('page');
        var repId = getOwnershipActorId(store.getState().auth.session) || store.getState().auth.session.id;
        getVisitTimeline(api, repId).then(function(tl) {
          store.update(function(d) { d.runtime.rep.timeline = Array.isArray(tl) ? tl : []; }, { dirty: ['page'] });
        }).catch(function() {});
      }).catch(function(err) {
        notify(store, 'error', 'تعذر إنهاء الزيارة', err?.message || '');
      });
      return;
    }
  ...
  document.addEventListener('input', (event) => {
    ...
    if (target.id === 'searchInput' || target.id === 'globalSearchInput') {
      ...
      scheduleSearch(schedule, target.id === 'globalSearchInput');
      return;
    }
    if (target.getAttribute('data-role') === 'ops-product-search') {
      store.patch({ ui: { ...state.ui, opsProductSearch: target.value } }, { silent: true });
      schedule('page');
      return;
    }
  ...
  document.addEventListener('submit', async (event) => {
    ...
    if (formType === 'login') {
      ...
      try {
        ...
        store.patch({
          ...
          runtime: {
            ...
            manager: {
              ...
              teamOrders: [],
              summary: {
                customers: 0, reps: 0, orders: 0,
                pending: 0, reviewing: 0, preparing: 0,
                dispatched: 0, delivered: 0, collected: 0,
        ...
        if (headerEl) patchAuthVisualState(headerEl, store.getState());
        persistSessionRecord(session);
        setOpsActorId(session?.id);
        persistSelectedCustomer(null);
        notify(store, 'success', 'تم الدخول', session.name || session.username || '');
        scheduleHydration(HYDRATION_PRIORITY.LOW, function() {
        ...
        if (loginSucceeded || persistedSession) {
          ...
          if (recoveredSession) {
            ...
            var headerEl2 = document.getElementById('appHeader');
            if (headerEl2) patchAuthVisualState(headerEl2, store.getState());
            setOpsActorId(recoveredSession?.id);
            persistSelectedCustomer(null);
            clearPendingFlow(store);
            navigateAuthority(store, 'home');
    ...
    if (formType === 'register') {
      ...
      try {
        ...
        store.patch({ auth: { ...store.getState().auth, session, selectedCustomer: null }, ui: { ...store.getState().ui, activeModal: null } }, { silent: true });
        var headerEl3 = document.getElementById('appHeader');
        if (headerEl3) patchAuthVisualState(headerEl3, store.getState());
        persistSelectedCustomer(null);
    ...
    if (formType === 'ops-product-edit') {
      ...
      try {
        if (!isNew) {
          ...
          const priceInputs = document.querySelectorAll('#opsEditUnits [data-unit-price]');
          for (const input of priceInputs) {
            const unit = input.getAttribute('data-unit-price');
            const tier = input.getAttribute('data-tier');
            const val = parseFloat(input.value);
            if (!unit || !tier || isNaN(val)) continue;
            await updateOpsProductUnitPrice(api, productId, unit, tier, val).catch(() => {});
          }
          const stockInputs = document.querySelectorAll('#opsEditUnits [data-unit-stock]');
          for (const input of stockInputs) {
            const unit = input.getAttribute('data-unit-stock');
            const val = parseInt(input.value, 10);
            if (!unit || isNaN(val)) continue;
            await updateOpsProductUnitStock(api, productId, unit, val).catch(() => {});
          }
          notify(store, 'success', 'تم التحديث', payload.product_name);
        } else {
          const created = await createOpsProduct(api, payload);
          ...
          if (newId) {
            ...
            for (const input of priceInputs) {
              ...
              const tier = input.getAttribute('data-tier');
              const val = parseFloat(input.value);
              if (!unit || !tier || isNaN(val) || val <= 0) continue;
              await addProductUnit(api, newId, unit, tier, val, 0).catch(() => {});
            }
          }
          notify(store, 'success', 'تم إنشاء المنتج', payload.product_name);
        }
        store.patch({ ui: { ...store.getState().ui, opsProductEditId: null, opsProductSearch: '' } });
        void loadOpsProductsIntoState(store, api);
        schedule('page');
      } catch (error) {
    ...
    if (formType === 'customer-create') {
      ...
      const payload = {
        ...
        owner_id: isSalesRepSession(session) ? ownershipActorId : null,
        is_active: true,
      };
...
async function loadRepIntoState(store, api) {
  ...
  try {
    const repId = getOwnershipActorId(session) || session.id;
    const [customers, orders, visits] = await Promise.all([
      loadRepCustomers(api, session),
      loadRepOrders(api, session),
      loadRepVisits(api, repId),
    ]);
    const timeline = await getVisitTimeline(api, repId).catch(function() { return []; });
    const activeVisit = await findActiveVisit(api, repId, null).catch(function() { return null; });
    store.update((draft) => {
      draft.runtime.rep = {
    ...
    store.update((draft) => {
      draft.runtime.rep = { loaded: false, loading: false, error: error?.message || 'LOAD_FAILED', customers: [], orders: [], visits: [], timeline: [], activeVisit: null };
    }, { dirty: ['page'] });
  }
}
...
async function loadCustomersIntoState(store, api, session = null) {
  ...
  if (hasOperationalAccess(rep)) {
    ...
    const rows = await loadAuthorizedCustomers(api, rep);
...
async function performCheckout(store, api, schedule) {
  ...
  try {
    ...
    var activeVisit = store.getState().runtime.rep.activeVisit;
    if (activeVisit && result.order) {
      updateActiveVisitOrder(api, activeVisit.id, result.order.id);
    }
    const invoice = {
      id: result.order.id,
...
export async function bootstrapApp() {
  ...
  const scheduler = createRenderLoop({
    header: () => { if (!nodes.header) return; return renderHeader(nodes.header, store.getState()); },
    ...
    search: () => { if (!isRuntimeInteractive(store.getState())) return; if (!nodes.search) return; return renderSearchBar(nodes.search, { value: store.getState().ui.search || '' }); },
    ...
    searchResults: () => renderGlobalSearchResults(store, api, isRuntimeInteractive),
  });

  function scheduleFullPage() {
  ...
  function renderContent() {
    ...
    if (booting) {
      if (phase === RUNTIME_PHASES.FAILED) {
        var msg = state.app.lastError || state.runtime?.lifecycle?.error || 'تعذر تهيئة النظام';
        if (nodes.page) nodes.page.innerHTML = '<section class="page-section"><div class="empty-state">' + msg + '</div></section>';
      } else {
        if (nodes.page) nodes.page.innerHTML = '<section class="page-section"><div class="skeleton-shell"><div class="skeleton-block skeleton-block--hero"></div><div class="skeleton-block skeleton-block--card"></div><div class="skeleton-block skeleton-block--card"></div><div class="skeleton-block skeleton-block--card"></div></div></section>';
      }
    ...
    if (state.app.route.name === 'rep') {
      const repModule = state.app.route.params?.module || 'dashboard';
      let repHtml = '';
      if (repModule === 'customers') repHtml = renderRepCustomersPage(state);
      else if (repModule === 'orders') repHtml = renderRepOrdersPage(state);
      else if (repModule === 'invoices') repHtml = renderRepInvoicesPage(state);
      else repHtml = renderRepDashboardPage(state);
      if (nodes.page) nodes.page.innerHTML = repHtml;
      if (nodes.opsNav) nodes.opsNav.innerHTML = renderRepNavigation(state);
      syncBodyShellHeight();
      applyBodyFlags();
      return;
    }
  ...
  store.subscribe((_, meta = {}) => {
    const dirty = Array.isArray(meta.dirty) && meta.dirty.length ? meta.dirty : [];
    if (dirty.length) scheduler.schedule(...dirty);
  });
  ...
  window.addEventListener('hashchange', () => {
    ...
    if (shellChanged) {
      ...
      scheduler.schedule('banner', 'theme', 'search', 'hero', 'footer');
    }
    ...
    if (nextRoute.name === 'invoice' && nextRoute.params?.invoiceId) {
      const invExists = (store.getState().commerce.invoices || []).some((inv) => String(inv.id) === String(nextRoute.params.invoiceId));
      if (!invExists) {
        store.update((draft) => { draft.runtime.loading.invoices = true; }, { silent: true });
        void loadInvoicesIntoState(store, api);
      }
    }
    scheduler.schedule('page', 'header', 'opsNav', 'repNav', 'drawer', 'modals', 'toast', 'adminHeader', 'adminSidebar', 'adminPage');
  });
  ...
  if (bootRoute.name === 'invoice' && bootRoute.params?.invoiceId) {
    store.update((draft) => { draft.runtime.loading.invoices = true; }, { silent: true });
    void loadInvoicesIntoState(store, api);
  }
  ...
  void (async () => {
    ...
    scheduleHydration(HYDRATION_PRIORITY.IDLE, function() {
      if (initialCompanyId) {
    ...
    if (isAdminOnlySession(session)) {
      ...
      if (isSalesRepSession(session)) {
        store.update((draft) => { draft.runtime.loading.customers = true; }, { silent: true });
        scheduleHydration(HYDRATION_PRIORITY.LOW, function() { void loadCustomersIntoState(store, api, session); });
      }
      store.update((draft) => { draft.runtime.loading.invoices = true; }, { silent: true });
      scheduleHydration(HYDRATION_PRIORITY.LOW, function() { void loadInvoicesIntoState(store, api); });
    }
    if (isSalesRepSession(session) && !isAdminOnlySession(session)) {
      scheduleHydration(HYDRATION_PRIORITY.IDLE, function() {
