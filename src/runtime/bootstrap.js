...
function bindInteractions(store, api, schedule) {
  document.addEventListener('click', async (event) => {
    ...
    if (action === 'download-invoice-pdf') {
      const raw = target.getAttribute('data-invoice-data');
      if (!raw) return;
      try {
        const { invoice, items, groupedItems } = JSON.parse(decodeURIComponent(raw));
        const companies = store.getState().commerce.catalog.companies || [];
        generateInvoicePDF(invoice, groupedItems, companies);
      } catch (e) {
        void e;
      }
      return;
...
async function loadInvoicesIntoState(store, api) {
  ...
  if (hasOperationalAccess(session)) {
    ...
    const filters = [`owner_id.eq.${ownershipActorId}`, `sales_rep_id.eq.${ownershipActorId}`, `rep_id.eq.${ownershipActorId}`];
...
export async function bootstrapApp() {
  ...
  function renderGlobalSearchResults(store, nodes) {
    ...
    const matchedProducts = allProducts.filter((p) => String(p.product_name || '').toLowerCase().includes(ql) || String(p.product_id || '').toLowerCase().includes(ql)).slice(0, 4);
    const matchedCustomers = customers.filter((c) => String(c.name || c.business_name || '').toLowerCase().includes(ql) || String(c.phone || '').includes(q)).slice(0, 3);
    const matchedInvoices = invoices.filter((inv) => String(inv.order_number || inv.invoice_number || inv.id || '').toLowerCase().includes(ql)).slice(0, 3);
    const allMatched = [...matchedProducts, ...matchedCustomers, ...matchedInvoices];
    if (!allMatched.length) { el.classList.add('is-hidden'); el.innerHTML = ''; return; }
    el.classList.remove('is-hidden');
    const html = [
      ...matchedProducts.map((p) => `<div class="header-search__result-item" data-action="global-search-select" data-product-id="${dom.escape(String(p.product_id))}"><span class="header-search__result-thumb"></span><span class="header-search__result-name">${dom.escape(p.product_name || '—')}</span><span class="header-search__result-meta">${dom.escape(p.company_name || 'منتج')}</span></div>`),
      ...matchedCustomers.map((c) => `<div class="header-search__result-item" data-action="select-customer" data-customer-id="${dom.escape(String(c.id))}"><span class="header-search__result-thumb"></span><span class="header-search__result-name">${dom.escape(c.name || c.business_name || '—')}</span><span class="header-search__result-meta">${dom.escape(c.phone || 'عميل')}</span></div>`),
      ...matchedInvoices.map((inv) => `<div class="header-search__result-item" data-action="view-invoice" data-invoice-id="${dom.escape(String(inv.id || inv.order_number || inv.invoice_number || ''))}"><span class="header-search__result-thumb"></span><span class="header-search__result-name">فاتورة #${dom.escape(String(inv.order_number || inv.invoice_number || inv.id || ''))}</span><span class="header-search__result-meta">${dom.escape(String(inv.total_amount ? formatMoney(inv.total_amount) : ''))}</span></div>`),
    ].join('');
    el.innerHTML = html;
  }
