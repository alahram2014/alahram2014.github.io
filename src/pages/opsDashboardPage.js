...
function renderQueueItem(item) {
  ...
  const noActions = !primary && item.noActionReason;

  const repBadge = item.repName
    ? `<span class="chip chip--rep">مندوب: ${dom.escape(item.repName)}</span>`
    : '';

  const ageBadge = item.orderAgeLabel
    ? `<span class="chip chip--age">${dom.escape(item.orderAgeLabel)}</span>`
    : '';

  const priorityBadge = item.priorityLevel
  ...
  return `
    <article class="ops-queue-card__item">
      ...
      <div class="ops-queue-card__item-actions">
        ${primary
          ? `<button class="btn btn--primary" type="button" data-action="workflow-transition" data-order-id="${dom.escape(String(item.id || ''))}" data-next-state-key="${dom.escape(String(primary.to_state_key || ''))}">ابدأ ${dom.escape(primary.to_state_label || 'التنفيذ')}</button>
             ${transitions.length > 1 ? `<span class="chip">+${transitions.length - 1}</span>` : ''}`
          : noActions
            ? `<span class="badge badge--muted">${dom.escape(item.noActionReason)}</span>`
            : '<span class="badge badge--muted">لا توجد إجراءات</span>'
        }
  ...
  return `
    <section class="page-section ops-section">
      ...
      <div class="ops-execution-list">
        ${cards.map((card) => {
          const repBadge = card.repName
            ? `<span class="chip chip--rep">مندوب: ${dom.escape(card.repName)}</span>`
            : '';
          const ageBadge = card.orderAgeLabel
            ? `<span class="chip chip--age">${dom.escape(card.orderAgeLabel)}</span>`
            : '';
          const priorityBadge = card.priorityLevel
            ? renderPriorityBadge(card.priorityLevel)
            : '';
          const noActions = !card.canExecute && card.noActionReason;
          ...
          return `
            <article class="ops-execution-card">
              ...
              <div class="ops-execution-card__footer">
                ${card.canExecute
                  ? `<button class="btn btn--primary" type="button" data-action="workflow-transition" data-order-id="${dom.escape(String(card.id || ''))}" data-next-state-key="${dom.escape(String(card.nextStateKey || ''))}">ابدأ ${dom.escape(card.actionLabel || 'التنفيذ')}</button>`
                  : noActions
                    ? `<span class="badge badge--muted">${dom.escape(card.noActionReason)}</span>`
                    : '<span class="badge badge--muted">لا توجد إجراءات متاحة</span>'
                }
