...
function filterProducts(products, query) {
  ...
  return products.filter((p) => {
    return String(p.product_name || '').toLowerCase().includes(q)
      || String(p.product_code || p.product_id || '').toLowerCase().includes(q)
          ...
          <div class="ops-edit-grid">
            <div class="ops-edit-col">
              ...
              <label class="ops-field">
                ...
                <input type="text" name="product_code" value="${dom.escape(product?.product_code || pid)}" ${isNew ? '' : 'readonly'} placeholder="${isNew ? 'سيتم إنشاؤه تلقائياً' : ''}">
              </label>
            </div>
            <div class="ops-edit-col">
              <label class="ops-field">
  ...
  const rows = filtered.map((p) => {
    ...
    return `
      <tr class="${p.isActive ? '' : 'row--inactive'}">
        ...
        <td class="ops-cell--code"><code>${dom.escape(p.product_code || p.product_id)}</code></td>
