...
import { canAccessCustomerManagement, canAccessOperationalDashboard, hasOperationalAccess, normalizeSessionRecord } from '../services/authService.js';
import { canOpenOpsWorkspace } from '../services/opsDashboardService.js';
import { canViewCustomer, canViewInvoice } from '../services/ownershipService.js';
...
export function renderCustomersPage(state) {
  ...
  const rawCustomers = hasOperationalAccess(session)
    ? (managerCustomers.length ? managerCustomers : (state.commerce.customers || []))
    : (state.commerce.customers || []);
  const customers = rawCustomers.filter((c) => canViewCustomer(session, c));
...
export function renderInvoicesPage(state) {
  const session = normalizeSessionRecord(state.auth.session);
  const rawInvoices = state.commerce.invoices || [];
  const invoices = session ? rawInvoices.filter((inv) => canViewInvoice(session, inv)) : [];
  return `
    <div class="page-stack">
      <section class="page-section">
        <div class="page-section__head"><div><h2>فواتيري</h2><p>الطلبات السابقة</p></div></div>
        <div class="invoice-list">${invoices.length ? invoices.map(invoiceCard).join('') : '<div class="empty-state">لا توجد فواتير</div>'}</div>
      </section>
    </div>
  `;
}
