...
export function renderFooter(container, state) {
  ...
  const cache = container.__footerCache;
  if (cache && cache.routeName === routeName && cache.cartAmount === cartAmount && cache.label === label) return;
  ...
  `;
  container.__footerCache = { routeName: routeName, cartAmount: cartAmount, label: label };
}
