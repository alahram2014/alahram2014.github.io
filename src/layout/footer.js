...
export function renderFooter(container, state) {
  ...
  container.innerHTML = `
    <nav class="footer-nav" aria-label="التنقل السفلي">
      ...
      <button type="button" data-action="open-cart-drawer" class="footer-nav__item footer-nav__item--cart">${cartAmount > 0 ? `<span class="footer-cart-amount">${dom.escape(cartAmount.toLocaleString('en-US'))}</span>` : '<span class="footer-cart-icon">🛒</span>'}</button>
    </nav>
  `;
}
