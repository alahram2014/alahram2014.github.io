...
export function renderLoginModal(state) {
  const isOpen = state.ui.activeModal === 'login' && !state.auth.session;
  return `
    <section class="modal-overlay modal-overlay--center ${isOpen ? 'is-open' : 'is-hidden'}" data-modal="login" role="dialog" aria-modal="true" tabindex="-1">
      ${modalFrame('تسجيل الدخول', `
        <form class="auth-form auth-form--modal" data-form="login">
          ...
          <button class="btn btn--primary" type="submit" ${state.auth.loginBusy ? 'disabled' : ''}>${state.auth.loginBusy ? 'جارٍ الدخول…' : 'دخول'}</button>
...
export function renderCustomerModal(state) {
  const locationDraft = state.ui.customerLocationDraft || {};
  const locationText = String(locationDraft.text || '').trim();
  const latValue = locationDraft.lat === null || locationDraft.lat === undefined ? '' : String(locationDraft.lat);
  const lngValue = locationDraft.lng === null || locationDraft.lng === undefined ? '' : String(locationDraft.lng);
  return `
    <section class="modal-overlay modal-overlay--center ${state.ui.activeModal === 'customer' ? 'is-open' : 'is-hidden'}" data-modal="customer" role="dialog" aria-modal="true" tabindex="-1">
      ${modalFrame('إضافة عميل', `
        <form class="auth-form auth-form--modal" data-form="customer-create">
          ...
          <label><span>الموقع</span><input name="location" type="text" value="${dom.escape(locationText)}" placeholder="سيتم تعبئته تلقائيًا أو يدويًا" /></label>
          <input type="hidden" name="location_lat" value="${dom.escape(latValue)}" />
          <input type="hidden" name="location_lng" value="${dom.escape(lngValue)}" />
          <div class="auth-form__actions">
            <button class="btn btn--ghost" type="button" data-action="capture-customer-location" ${state.ui.customerLocationBusy ? 'disabled' : ''}>${state.ui.customerLocationBusy ? 'جارٍ تحديد الموقع…' : 'إضافة موقعي الحالي'}</button>
          </div>
          ${state.ui.customerLocationError ? `<div class="empty-state">${dom.escape(state.ui.customerLocationError)}</div>` : ''}
            ...
            <div class="product-modal__content">
              <div class="product-modal__meta">
                ...
                <div class="badge-row">
                  ...
                  ${availLabel ? `<span class="availability-badge ${badgeClass}">${availLabel}</span>` : ''}
                </div>
              </div>
              <div class="product-modal__units">${units.map((unit) => {
                const uAvail = unitAvailability(unit);
                const uDisabled = uAvail !== AVAILABILITY.AVAILABLE;
                return `<button class="unit-chip ${unit.unit_code === selectedUnit ? 'is-active' : ''}" type="button" data-action="set-unit" data-product-id="${dom.escape(product.product_id)}" data-unit="${dom.escape(unit.unit_code)}" ${uDisabled ? 'disabled' : ''}>${dom.escape(labelForUnit(unit.unit_code))}</button>`;
              }).join('')}</div>
              <div class="product-modal__pricing">
                ...
                <div class="product-modal__price-row"><span class="price price--main">${formatMoney(Number(currentUnit?.final_price ?? display.final ?? 0))} ج.م</span><span class="unit-label">${canBuy ? 'جاهز للشراء' : (availLabel || 'غير متاح حاليًا')}</span></div>
              </div>
            </div>
          </div>
        </div>
      `, `<button class="btn btn--primary" type="button" data-action="toggle-product" data-product-id="${dom.escape(product.product_id)}" ${canBuy ? '' : 'disabled'}>${canBuy ? 'شراء' : (availLabel || 'غير متاح')}</button>`)}
    </section>
  `;
}
    ...
    <section class="modal-overlay modal-overlay--center is-open" data-modal="invoice" role="dialog" aria-modal="true" tabindex="-1">
      ${modalFrame(`فاتورة #${invoice.order_number || invoice.invoice_number || invoice.id}`, `
        <div class="invoice-preview">
          <div class="invoice-preview__row"><span>التاريخ</span><strong>${new Intl.DateTimeFormat('ar-EG', { dateStyle: 'medium', timeStyle: 'short' }).format(new Date(invoice.created_at || Date.now()))}</strong></div>
