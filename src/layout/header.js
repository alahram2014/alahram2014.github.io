...
function preserveSearchInput(container) {
...
function restoreSearchInput(container, state) {
  if (!state) return;
  const input = container.querySelector('#globalSearchInput');
  if (!input) return;
...
export function renderHeader(container, state) {
  ...
  const savedSearch = preserveSearchInput(container);
  ...
  restoreSearchInput(container, savedSearch);
}
