...
export function renderHeader(container, state) {
  ...
  if (existingShell) {
    ...
    if (isFocused) {
      ...
      if (menuEl) { menuEl.outerHTML = buildMenuHtml(session, canOpenDashboard, isRep, canOpenCustomers, state.ui.desktopMode, state.ui.accountMenuOpen); }
      return;
    }
    ...
    if (menuEl2) { menuEl2.outerHTML = buildMenuHtml(session, canOpenDashboard, isRep, canOpenCustomers, state.ui.desktopMode, state.ui.accountMenuOpen); }
    return;
  }
