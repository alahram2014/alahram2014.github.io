...
export function patchAuthVisualState(container, state) {
  ...
  if (menuEl) {
    ...
    menuEl.className = 'header-menu' + (state.ui.accountMenuOpen ? ' is-open' : '');
    menuEl.innerHTML = buildMenuButtons(session, canOpenDashboard, isRep, canOpenCustomers);
  }
}
...
export function renderHeader(container, state) {
  ...
  if (cache && cache.routeName === routeName && cache.sessionLabel === sessionLabel && cache.canOpenDashboard === canOpenDashboard && cache.isRep === isRep && cache.tierName === (tier?.tier_name || '') && cache.desktopMode === state.ui.desktopMode && cache.canOpenCustomers === canOpenCustomers) {
    ...
    if (menuEl && cache.accountMenuOpen !== state.ui.accountMenuOpen) {
      menuEl.className = 'header-menu' + (state.ui.accountMenuOpen ? ' is-open' : '');
      menuEl.innerHTML = buildMenuButtons(session, canOpenDashboard, isRep, canOpenCustomers);
    }
    container.__headerCache = { routeName: routeName, sessionLabel: sessionLabel, canOpenDashboard: canOpenDashboard, isRep: isRep, tierName: (tier?.tier_name || ''), accountMenuOpen: state.ui.accountMenuOpen, desktopMode: state.ui.desktopMode, canOpenCustomers: canOpenCustomers };
    return;
  }
  ...
  if (existingShell) {
    ...
    if (isFocused) {
      ...
      if (clearBtn) { clearBtn.classList.toggle('is-hidden', !searchValue); }
      var menuEl = container.querySelector('[data-role="account-menu"]');
      if (menuEl) { menuEl.className = 'header-menu' + (state.ui.accountMenuOpen ? ' is-open' : ''); menuEl.innerHTML = buildMenuButtons(session, canOpenDashboard, isRep, canOpenCustomers); }
      container.__headerCache = { routeName: routeName, sessionLabel: sessionLabel, canOpenDashboard: canOpenDashboard, isRep: isRep, tierName: (tier?.tier_name || ''), accountMenuOpen: state.ui.accountMenuOpen, desktopMode: state.ui.desktopMode, canOpenCustomers: canOpenCustomers };
      return;
    }
    ...
    if (menuEl2) { menuEl2.className = 'header-menu' + (state.ui.accountMenuOpen ? ' is-open' : ''); menuEl2.innerHTML = buildMenuButtons(session, canOpenDashboard, isRep, canOpenCustomers); }
    container.__headerCache = { routeName: routeName, sessionLabel: sessionLabel, canOpenDashboard: canOpenDashboard, isRep: isRep, tierName: (tier?.tier_name || ''), accountMenuOpen: state.ui.accountMenuOpen, desktopMode: state.ui.desktopMode, canOpenCustomers: canOpenCustomers };
    return;
  }
...
function buildMenuButtons(session, canOpenDashboard, isRep, canOpenCustomers) {
  if (session) {
    return (
      '<button type="button" data-action="pwa-install">📲 تثبيت التطبيق</button>' +
      '<button type="button" data-action="update-app">🔄 تحديث التطبيق</button>' +
...
function buildMenuHtml(session, canOpenDashboard, isRep, canOpenCustomers, desktopMode, accountMenuOpen) {
  return '<div class="header-menu ' + (accountMenuOpen ? 'is-open' : '') + '" data-role="account-menu">' + buildMenuButtons(session, canOpenDashboard, isRep, canOpenCustomers) + '</div>';
}
