...
export function invoiceCard(invoice) {
  ...
  const workflowLabel = formatStatus(invoice.workflow_state_key || invoice.workflow_status || invoice.status);
  const customerTypeLabel = labelForCustomerType(invoice.customer_type || invoice.user_type || 'direct');
  const customerPhone = invoice.customer_phone || invoice.phone || '';
  const supportWhatsapp = invoice.support_whatsapp || '';

  const items = Array.isArray(invoice.items) ? invoice.items : [];
  const itemRows = items.slice(0, 5).map((item) => {
    const name = safeText(item.product_name || item.title || item.name || '');
    const unit = safeText(item.unitLabel || item.unit || 'قطعة');
    const qty = Number(item.qty || 1);
    const price = Number(item.price || 0);
    const total = qty * price;
    return `<tr><td>${dom.escape(name)}</td><td>${dom.escape(unit)}</td><td>${qty}</td><td>${formatMoney(price)}</td><td>${formatMoney(total)}</td></tr>`;
  }).join('');
  ...
  return `
    <article class="invoice-card">
      <div class="invoice-card__top">
        ...
        <strong class="invoice-card__total">${formatMoney(invoice.total_amount || 0)} ج.م</strong>
      </div>
      <div class="invoice-card__meta">
        <span class="chip">${dom.escape(customerTypeLabel)}</span>
        <span class="chip">${dom.escape(workflowLabel)}</span>
        <button class="btn btn--ghost invoice-card__action" type="button" data-action="view-invoice" data-invoice-id="${dom.escape(invoiceId)}">عرض</button>
        ${supportWhatsapp ? `<button class="btn btn--ghost invoice-card__action" type="button" data-action="whatsapp-invoice" data-invoice-id="${dom.escape(invoiceId)}" data-whatsapp="${dom.escape(supportWhatsapp)}" title="إرسال عبر واتساب">📱</button>` : ''}
      </div>
      ${items.length ? `<div class="invoice-card__items"><table class="invoice-items-table"><thead><tr><th>المنتج</th><th>الوحدة</th><th>الكمية</th><th>السعر</th><th>الإجمالي</th></tr></thead><tbody>${itemRows}</tbody></table>${hasMore ? `<div class="invoice-card__more">+${items.length - 5} أصناف أخرى</div>` : ''}<div class="invoice-card__grand-total"><span>الإجمالي</span><strong>${formatMoney(invoice.total_amount || 0)} ج.م</strong></div></div>` : ''}
    </article>
  `;
}
