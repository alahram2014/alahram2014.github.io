...
import { resolveWorkflowActions } from '../services/workflowService.js';
import { unitAvailability, canAddProductToCart, getAvailabilityLabel, getAvailabilityBadgeClass, AVAILABILITY } from '../runtime/modules/availabilityRuntime.js';
...
function labelForCustomerType(value) {
  const type = safeText(value, 'direct').toLowerCase();
  if (type === 'rep') return 'عميل تابع';
  if (type === 'direct') return 'عميل مباشر';
  return safeText(value, 'عميل');
}

function labelForOwnerType(value) {
  const type = safeText(value, '').toLowerCase();
  if (type === 'sales_rep') return 'مندوب';
  if (type === 'sales_manager') return 'مدير بيع';
  if (type === 'admin') return 'إدارة';
  return safeText(value, '');
}
...
function renderUnitChips(product, selectedUnit) {
  ...
  return units.map((unit) => {
    ...
    const isDisabled = unitAvailability(unit) !== AVAILABILITY.AVAILABLE;
    return `<button class="unit-chip ${active ? 'is-active' : ''}" data-action="set-unit" data-product-id="${dom.escape(safeId(product?.product_id))}" data-unit="${dom.escape(safeText(unit.unit_code))}" ${isDisabled ? 'disabled' : ''}>${dom.escape(labelForUnit(unit.unit_code))}</button>`;
  }).join('');
}
...
export function productCard(product, tier, { unit, qty, inCart } = {}) {
  ...
  const canBuy = canAddProductToCart(product, selectedUnit);
  const quantity = Math.max(1, Number(qty || 1));
  const image = safeImageMarkup(product.product_image, productName, productName);
  const availState = unitAvailability(currentUnit);
  const availLabel = getAvailabilityLabel(availState);
  const badgeClass = getAvailabilityBadgeClass(availState);

  const ctaLabel = !canBuy ? (inCart ? 'إزالة من السلة' : availLabel || 'شراء') : inCart ? 'إزالة من السلة' : 'شراء';
  ...
  return `
    <article class="product-card ${canBuy ? '' : 'product-card--disabled'}" data-product-id="${dom.escape(productId)}">
      ...
      <div class="product-card__body">
        ...
        <div class="product-card__meta">${dom.escape(safeText(product.company_name, ''))}${availLabel ? `<span class="availability-badge ${badgeClass}">${availLabel}</span>` : ''}</div>
...
export function invoiceCard(invoice) {
  ...
  const dateStr = new Intl.DateTimeFormat('ar-EG', { dateStyle: 'medium' }).format(created);
  const timeStr = new Intl.DateTimeFormat('ar-EG', { timeStyle: 'short' }).format(created);
  const invoiceNum = safeText(invoice.order_number || invoice.invoice_number || invoice.id);

  return `
    <article class="invoice-card" data-action="view-invoice" data-invoice-id="${dom.escape(invoiceId)}">
      <div class="invoice-card__head">
        <strong>فاتورة رقم ${dom.escape(invoiceNum)}</strong>
        <span class="chip">${dom.escape(formatStatus(invoice.workflow_state_key || invoice.workflow_status || invoice.status))}</span>
      </div>
      <div class="invoice-card__customer">${dom.escape(safeText(invoice.customer_name || ''))}</div>
      <div class="invoice-card__meta">
        <span>${dom.escape(dateStr)}</span>
        <span>${dom.escape(timeStr)}</span>
        <span class="invoice-card__amount">${formatMoney(invoice.total_amount || 0)} ج.م</span>
      </div>
      <div class="invoice-card__actions">
        <button class="btn btn--ghost" type="button" data-action="view-invoice" data-invoice-id="${dom.escape(invoiceId)}">عرض الفاتورة</button>
      </div>
    </article>
  `;
}
...
export function renderWorkflowActionButtons(order, session, { compact = false } = {}) {
  ...
  if (!transitions.length) {
    return compact ? '' : '<div class="badge">لا توجد إجراءات متاحة</div>';
  }

  return `
    <div class="workflow-actions ${compact ? 'workflow-actions--compact' : ''}">
      ${transitions.map((transition, index) => {
        const label = transition.to_state_label || transition.to_state_key || 'تغيير الحالة';
        const isPrimary = index === 0;
        return `
          <button class="btn ${isPrimary ? 'btn--primary' : 'btn--ghost'} workflow-actions__btn ${isPrimary ? 'workflow-actions__btn--primary' : ''}" type="button" data-action="workflow-transition" data-order-id="${dom.escape(String(order.id || ''))}" data-next-state-key="${dom.escape(String(transition.to_state_key || ''))}">
            ${dom.escape(isPrimary ? `ابدأ ${label}` : label)}
          </button>
        `;
      }).join('')}
    </div>
  ...
  return `
    <article class="customer-card ${selected ? 'is-selected' : ''}" data-action="select-customer" data-customer-id="${dom.escape(customerId)}">
      ...
      <div class="customer-card__meta">
        <span class="chip">${dom.escape(labelForCustomerType(customer.customer_type || 'direct'))}</span>
        ${customer.owner_user_type ? `<span class="chip">${dom.escape(labelForOwnerType(customer.owner_user_type))}</span>` : ''}
      </div>
